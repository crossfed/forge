...
contract-manifest --wasm hello.wasm --abi hello.abi --imports intrinsics.json \
  ...
  --source-dependencies generated/hello.source-dependencies.txt \
...
source-graph descriptor, compiler-discovered contract includes and explicit
toolchain identity. Manifest schema v2
...
used only by build-local descriptors to read files and never appear in the
sidecar.
