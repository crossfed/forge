...
contract-manifest --wasm hello.wasm --abi hello.abi --imports intrinsics.json \
  --source-graph hello.contract-graph.json \
...
Inputs are the validated WASM/ABI pair, approved imports, the canonical
descriptor graph and explicit toolchain identity. The only output is the
deterministic `*.contract.json` sidecar.
