...
`forge_config_env` converts process environment variables and explicit `.env` files into
`forge::config::core::document`. It is a config source adapter, like
`forge_config_program_options` for CLI flags: plugins and applications publish schema
descriptors, `forge_config_env` maps prefixed variables to canonical config keys, and
...
Target: `forge_config_env`.

Dependencies: `forge_config_core`, `forge_schema`.
...
auto dotenv = forge::config::env::load_document(
   workspace / ".env",
   registry,
...
`forge_config_env` does not own precedence. Keep it explicit at the composition layer:
...
```cpp
auto parsed = forge::config::env::read<http_config>(
...
```cpp
auto vars = std::vector<forge::config::env::environment_variable>{
   {.name = "FORGE_APP_HTTP_BIND_PORT", .value = "9090"},
};

auto parsed = forge::config::env::read_variables(vars, registry, {.prefix = "FORGE_APP"});
```

### Generate `.env.example`

```cpp
auto example = forge::config::env::write_example(registry, {.prefix = "FORGE_APP"});
if (example.ok()) {
   forge::config::env::save_example(workspace / ".env.example", registry, {.prefix = "FORGE_APP"});
}
```
...
```cpp
auto redacted = forge::config::env::write_document(document, registry, {.prefix = "FORGE_APP"});
if (!redacted.ok()) {
...
`test_forge_config_env` covers env name mapping, flat sections, aliases, dotenv syntax,
