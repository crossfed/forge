...
export module forge.tooling.testing.schema;
...
template <> struct sort_key<forge::tooling::testing::float64> {
   [[nodiscard]] sort_key_bytes operator()(forge::tooling::testing::float64 value) const {
      auto bits = value.bits;
      const auto magnitude = bits & 0x7fff'ffff'ffff'ffffULL;
      if (magnitude > 0x7ff0'0000'0000'0000ULL) {
         FORGE_THROW_EXCEPTION(forge::tooling::testing::exceptions::database_error,
                               "NaN is not an ordered database key");
      }
      if (magnitude == 0U) {
         bits = 0U;
      }
...
template <> struct sort_key<forge::tooling::testing::float128> {
   [[nodiscard]] sort_key_bytes operator()(forge::tooling::testing::float128 value) const {
...
template <> struct index_for_id<forge::tooling::testing::table::id_t> {
   using type = forge::tooling::testing::table_index;
};

template <> struct index_for_id<forge::tooling::testing::key_value::id_t> {
   using type = forge::tooling::testing::key_value_index;
};

template <> struct index_for_id<forge::tooling::testing::index64::id_t> {
   using type = forge::tooling::testing::index64_index;
};

template <> struct index_for_id<forge::tooling::testing::index128::id_t> {
   using type = forge::tooling::testing::index128_index;
};

template <> struct index_for_id<forge::tooling::testing::index256::id_t> {
   using type = forge::tooling::testing::index256_index;
};

template <> struct index_for_id<forge::tooling::testing::index_double::id_t> {
   using type = forge::tooling::testing::index_double_index;
};

template <> struct index_for_id<forge::tooling::testing::index_long_double::id_t> {
   using type = forge::tooling::testing::index_long_double_index;
};
