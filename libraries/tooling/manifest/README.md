...
Target `forge_tooling_manifest`, package component `tooling_manifest`, writes
the deterministic `*.contract.json` runtime sidecar. Public modules are
`forge.tooling.manifest.generator` and
`forge.tooling.manifest.command`.

```cpp
import forge.tooling.manifest.generator;

forge::tooling::manifest::generate({
   .wasm = "token.wasm",
   .abi = "token.abi",
...
Clang and does not validate policy; use `tooling_validation` first.
