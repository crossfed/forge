...
template <> struct hash<forge::crypto::sha256> {
   std::size_t operator()(const forge::crypto::sha256& value) const noexcept {
      return static_cast<std::size_t>(value._hash[0]);
   }
};

inline std::ostream& operator<<(std::ostream& stream, const forge::crypto::sha256& value) {
...
template <> struct hash<forge::crypto::sha256> {
   std::size_t operator()(const forge::crypto::sha256& value) const noexcept {
      return forge::crypto::hash_value(value);
   }
};
