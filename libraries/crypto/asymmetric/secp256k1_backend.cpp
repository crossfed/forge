...
compact_signature private_key::sign_compact(const forge::crypto::digest::sha256& digest, bool require_canonical) const {
   FORGE_ASSERT(my->_key != empty_priv);
...
der_signature sign_der(const private_key& key, std::span<const std::uint8_t> message) {
   const auto digest = forge::crypto::digest::sha256::hash(message);
