...
template <typename Key> [[nodiscard]] std::vector<std::uint8_t> secret_bytes(const Key& value) {
   ...
   if constexpr (std::same_as<std::decay_t<decltype(secret)>, digest::sha256>) {
   ...
   return std::visit(
       [&](const auto& key) -> signature {
          ...
          if constexpr (std::same_as<key_type, secp256k1::private_key>) {
             return make_k1_signature(key.sign_compact(digest::sha256::hash(message), true));
          } else if constexpr (std::same_as<key_type, p256::private_key>) {
             return make_r1_signature(key.sign_compact(digest::sha256::hash(message)));
...
signature private_key::sign_digest(const digest::sha256& digest) const {
   return std::visit(
       [&](const auto& key) -> signature {
          using key_type = std::decay_t<decltype(key)>;
          if constexpr (std::same_as<key_type, secp256k1::private_key>) {
   ...
   return std::visit(
       [&](const auto& typed_key) {
          ...
          if constexpr (std::same_as<key_type, k1_public_key>) {
             ...
             return webauthn::recover(std::get<webauthn_signature>(value), digest::sha256::hash(message), true) == typed_key;
...
public_key recover(const signature& value, const digest::sha256& digest, bool check_canonical) {
   return std::visit(
