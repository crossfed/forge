...
namespace {
using forge::crypto::asymmetric::detail::bn_ctx;
using forge::crypto::asymmetric::detail::ec_group;
using forge::crypto::asymmetric::detail::ec_point;
using forge::crypto::asymmetric::detail::ecdsa_sig;
using forge::crypto::asymmetric::detail::ssl_bignum;
...
public_key_data point_to_public_key_data(const EC_POINT* point) {
   ...
   FORGE_ASSERT(written == result.size(), "unexpected P-256 public key size",
...
public_key_point_data point_to_uncompressed_data(const public_key_data& data) {
   ...
   FORGE_ASSERT(
       EC_POINT_oct2point(group, point, reinterpret_cast<const unsigned char*>(data.data()), data.size(), ctx));
...
std::vector<unsigned char> sign_der(const private_key_secret& secret, const forge::crypto::digest::sha256& digest) {
...
std::optional<public_key_data> recover_public_key_from_sig(ECDSA_SIG* sig, const forge::crypto::digest::sha256& digest,
                                                           int recid, bool check) {
...
compact_signature signature_from_ecdsa(const public_key_data& pub_data,
                                       forge::crypto::asymmetric::detail::ecdsa_sig& sig,
                                       const forge::crypto::digest::sha256& d) {
...
public_key public_key::mult(const forge::crypto::digest::sha256& digest) {
   const ec_group& group = get_curve();
   bn_ctx ctx(BN_CTX_new());
...
public_key public_key::add(const forge::crypto::digest::sha256& digest) const {
   try {
      const ec_group& group = get_curve();
      bn_ctx ctx(BN_CTX_new());
...
private_key private_key::generate_from_seed(const forge::crypto::digest::sha256& seed, const forge::crypto::digest::sha256& offset) {
   ...
   forge::crypto::digest::sha256 secret;
...
private_key private_key::regenerate(const forge::crypto::digest::sha256& secret) {
   private_key self;
   self.my->_key = secret;
...
forge::crypto::digest::sha256 private_key::get_secret() const {
   return my->_key;
}
...
private_key private_key::generate() {
   private_key self;
   do {
      forge::crypto::core::fill_random(
          std::span<std::uint8_t>{reinterpret_cast<std::uint8_t*>(self.my->_key.data()), self.my->_key.data_size()});
   } while (!valid_secret(self.my->_key));
   return self;
}

signature private_key::sign(const forge::crypto::digest::sha256& digest) const {
   const auto der = sign_der(my->_key, digest);
   signature sig;
   std::memset(sig.data(), 0, sig.size());
   FORGE_ASSERT(der.size() <= sig.size(), "P-256 DER signature is too large",
                forge::exceptions::ctx("der_size", der.size()), forge::exceptions::ctx("signature_size", sig.size()));
...
der_signature sign_der(const private_key& key, std::span<const std::uint8_t> message) {
   const auto der = sign_der(key.get_secret(), forge::crypto::digest::sha256::hash(message));
...
bool verify_der(const public_key& key, std::span<const std::uint8_t> message,
   ...
   return public_key(key.serialize()).verify(forge::crypto::digest::sha256::hash(message), value);
}
...
forge::crypto::digest::sha512 private_key::get_shared_secret(const public_key& other) const {
   ...
   return forge::crypto::digest::sha512::hash(reinterpret_cast<const char*>(secret.data()), secret.size());
}

private_key::~private_key() {}

public_key::public_key(const compact_signature& c, const forge::crypto::digest::sha256& digest, bool check_canonical)
    : my(std::make_unique<detail::public_key_impl>()) {
   my->_key = recover_public_key_data(c, digest, check_canonical);
}

compact_signature private_key::sign_compact(const forge::crypto::digest::sha256& digest) const {
   try {
      FORGE_ASSERT(valid_secret(my->_key));
