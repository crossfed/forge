...
module forge.crypto.math.bigint;
...
core::bytes bigint::to_bytes() const {
   auto to = core::bytes(BN_num_bytes(n));
   BN_bn2bin(n, to.data());
   return to;
}
...
void from_variant(const variant& v, bigint& bi) {
   if (v.is_numeric())
      ...
      auto bin = core::bytes(decoded.begin(), decoded.end());
      bi = bigint(bin);
   }
}
