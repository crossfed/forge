...
[[nodiscard]] core::bytes write_private_der(EVP_PKEY* key) {
...
[[nodiscard]] core::bytes write_public_der(EVP_PKEY* key) {
...
core::bytes read_block(std::string_view text, std::string_view label) {
   ...
   auto out = core::bytes(data, data + size);
   cleanup();
   return out;
}
