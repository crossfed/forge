...
[[nodiscard]] core::bytes write_spki(EVP_PKEY* key) {
   const auto length = i2d_PUBKEY(key, nullptr);
   if (length <= 0) {
      backend_error("failed to size public key DER");
   }
   auto out = core::bytes(static_cast<std::size_t>(length));
...
[[nodiscard]] core::bytes ec_public_octets(EVP_PKEY* key) {
...
template <typename Range> [[nodiscard]] core::bytes bytes_from_range(const Range& value) {
   auto out = core::bytes{};
...
[[nodiscard]] public_key read_ec_public(EVP_PKEY* key) {
   ...
   if (nid == NID_X9_62_prime256v1) {
      auto data = fixed_bytes<asymmetric::p256::public_key_point_data>(point, "P-256 public key");
      return asymmetric::r1_public_key{asymmetric::p256::public_key{data}.serialize()};
   }
   if (nid == NID_secp256k1) {
      auto data = fixed_bytes<asymmetric::secp256k1::public_key_point_data>(point, "secp256k1 public key");
      return asymmetric::k1_public_key{asymmetric::secp256k1::public_key{data}.serialize()};
   }
   invalid_key("unsupported EC public key curve");
}

[[nodiscard]] core::bytes raw_public_key(EVP_PKEY* key) {
   auto size = std::size_t{};
...
[[nodiscard]] core::bytes raw_private_key(EVP_PKEY* key) {
   auto size = std::size_t{};
...
[[nodiscard]] core::bytes uncompressed_ec_point(int nid, std::span<const std::uint8_t> compressed) {
...
[[nodiscard]] pkey_ptr make_ed25519_public_key(const asymmetric::ed25519::public_key_data& value) {
...
[[nodiscard]] digest::sha256 ec_private_scalar(EVP_PKEY* key) {
   BIGNUM* raw = nullptr;
...
public_key read_public_key(std::span<const std::uint8_t> value) {
   ...
   if (type == EVP_PKEY_RSA) {
      return asymmetric::rsa_public_key{forge::crypto::core::bytes{value.begin(), value.end()}};
   }
   if (type == EVP_PKEY_ED25519) {
      return asymmetric::ed25519_public_key{
          fixed_bytes<asymmetric::ed25519::public_key_data>(raw_public_key(key.get()), "Ed25519 public key")};
   }
   if (type == EVP_PKEY_EC) {
...
private_key read_private_key(std::span<const std::uint8_t> value) {
   ...
   if (type == EVP_PKEY_RSA) {
      return private_key{
          private_key::storage_type{
              asymmetric::rsa::private_key::regenerate(forge::crypto::core::bytes{value.begin(), value.end()})}};
   }
   if (type == EVP_PKEY_ED25519) {
      return private_key{private_key::storage_type{asymmetric::ed25519::private_key::regenerate(
          fixed_bytes<asymmetric::ed25519::private_key_secret>(raw_private_key(key.get()), "Ed25519 private key"))}};
   }
   if (type == EVP_PKEY_EC) {
      const auto scalar = ec_private_scalar(key.get());
      const auto nid = ec_curve_nid(key.get());
      if (nid == NID_X9_62_prime256v1) {
         return private_key{private_key::storage_type{asymmetric::p256::private_key::regenerate(scalar)}};
      }
      if (nid == NID_secp256k1) {
         return private_key{private_key::storage_type{asymmetric::secp256k1::private_key::regenerate(scalar)}};
      }
...
core::bytes write_public_key(const public_key& key) {
   return std::visit(
       [](const auto& value) -> core::bytes {
