...
[[nodiscard]] core::bytes write_der(X509* certificate) {
...
certificate::certificate(core::bytes der) : der_(std::move(der)) {}
...
const core::bytes& certificate::der() const noexcept {
   return der_;
}

core::bytes certificate::public_key_der() const {
...
core::bytes certificate::extension(std::string_view oid) const {
   auto parsed = parse_der(der_);
   ...
   return core::bytes(data->data, data->data + data->length);
}
