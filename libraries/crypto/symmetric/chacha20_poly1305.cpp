...
module forge.crypto.symmetric.chacha20_poly1305;
...
core::bytes encrypt(const key& key, const nonce& nonce, std::span<const std::uint8_t> associated_data,
   ...
   auto out = core::bytes(plaintext.size() + 16U);
   auto written = 0;
   if (!plaintext.empty() &&
...
core::bytes decrypt(const key& key, const nonce& nonce, std::span<const std::uint8_t> associated_data,
   ...
   auto out = core::bytes(payload_size);
   auto written = 0;
   if (payload_size != 0 &&
