...
void emit_chunk(const aes_byte_sink& sink, core::bytes& chunk, int size) {
   if (size > 0) {
...
aes256_gcm_authentication aes256_gcm_encoder::finalize() {
   ...
   auto tag = core::bytes(aes_gcm_tag_size);
...
aes256_cbc_ciphertext encrypt_aes256_cbc(const aes256_cbc_encrypt_request& request) {
   ...
   auto out = core::bytes(request.plaintext.size() + aes_cbc_iv_size);
   auto out_size = int{};
   auto total_size = int{};
...
core::bytes decrypt_aes256_cbc(const aes256_cbc_decrypt_request& request) {
