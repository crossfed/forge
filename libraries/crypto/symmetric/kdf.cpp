...
[[nodiscard]] core::bytes hmac_sha256(std::span<const std::uint8_t> key, std::span<const std::uint8_t> input) {
   auto out = core::bytes(EVP_MAX_MD_SIZE);
   auto out_size = 0U;
...
core::bytes derive_hkdf_sha256(const hkdf_sha256_span_request& request) {
   ...
   if (request.salt.empty()) {
      const auto zero_salt = core::bytes(32, 0);
