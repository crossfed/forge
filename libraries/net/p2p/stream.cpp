...
stream::stream(forge::net::transport::stream value, std::vector<std::uint8_t> buffered) : stream{std::move(value)} {
...
boost::asio::awaitable<forge::net::transport::chunk> stream::async_read_chunk() {
   ...
   if (!available_bytes(impl_->buffer, impl_->consumed).empty()) {
      auto out = forge::net::transport::chunk{available_bytes(impl_->buffer, impl_->consumed)};
      impl_->buffer.clear();
      impl_->consumed = 0;
      co_return out;
   }
   ...
   while (true) {
      ...
      if (decoded.status == forge::net::transport::frame_decode_status::complete) {
         ...
         if (impl_->consumed == 0 && decoded.consumed == impl_->buffer.size()) {
            ...
            impl_->buffer.clear();
            impl_->consumed = 0;
            co_return forge::net::transport::chunk{std::move(storage), payload_offset, payload_size};
         }
...
forge::net::transport::stream stream::into_transport_stream() && {
   ...
   impl_.reset();
   return forge::net::transport::detail::stream_access::with_buffer(std::move(transport), std::move(buffered));
}
