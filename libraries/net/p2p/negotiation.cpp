...
module forge.net.p2p.negotiation;
...
namespace forge::net::p2p::protocol_negotiation {
namespace {
...
class stream_io {
 public:
   stream_io(forge::net::transport::stream stream_value, options opts_value)
   ...
   [[nodiscard]] forge::net::p2p::stream finish() {
      return forge::net::p2p::stream{std::move(stream_), std::move(buffer_)};
   }
...
class p2p_stream_io {
 public:
   p2p_stream_io(forge::net::p2p::stream stream_value, options opts_value)
       : stream_(std::move(stream_value)), opts_(opts_value) {}
...
boost::asio::awaitable<forge::net::p2p::stream> async_select(forge::net::transport::stream raw, protocol_id protocol, options opts) {
   require_protocol_id(protocol);
...
boost::asio::awaitable<forge::net::p2p::stream> async_select(forge::net::p2p::stream raw, protocol_id protocol, options opts) {
   require_protocol_id(protocol);
...
boost::asio::awaitable<negotiated_stream> async_accept(forge::net::transport::stream raw, std::vector<protocol_id> protocols,
                                                       options opts) {
   if (protocols.size() > opts.max_protocols) {
...
boost::asio::awaitable<negotiated_stream> async_accept(forge::net::p2p::stream raw, std::vector<protocol_id> protocols,
                                                       options opts) {
   if (protocols.size() > opts.max_protocols) {
      FORGE_THROW_EXCEPTION(exceptions::codec_error, "too many local multistream-select protocols");
   }
