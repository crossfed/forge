...
transaction_participant_impl::transaction_participant_impl(
   ...
   digest namespace_hash)
    : name_{"forge.db.authenticated:" + std::move(domain)},
...
transaction_participant_impl::classify(
...
transaction_participant_impl::make_retention_guard(
   ...
   if (mutation.column_family.name != family_.name ||
       mutation.key.bytes() !=
          detail::latest_key(namespace_hash_).bytes() ||
       !mutation.before || token.empty()) {
      return std::nullopt;
   }
   const auto previous = detail::decode_root(*mutation.before);
   return forge::db::core::record_address{
      .column_family = family_,
      .key = detail::retention_guard_key(
         namespace_hash_, previous.version, token),
   };
}

boost::asio::awaitable<void>
transaction_participant_impl::prepare_savepoint(
   forge::db::core::savepoint_id_t id) {
   pending_savepoint_ = savepoint_frame{.id = id, .staged = staged_};
   co_return;
}

void transaction_participant_impl::publish_savepoint(
   forge::db::core::savepoint_id_t id) noexcept {
...
void transaction_participant_impl::discard_savepoint(
   forge::db::core::savepoint_id_t id) noexcept {
   if (pending_savepoint_ && pending_savepoint_->id == id) {
      pending_savepoint_.reset();
   }
...
boost::asio::awaitable<void>
transaction_participant_impl::rollback_to_savepoint(
   forge::db::core::savepoint_id_t id,
...
boost::asio::awaitable<void>
transaction_participant_impl::release_savepoint(
   forge::db::core::savepoint_id_t id,
   forge::db::core::participant_access&) {
   if (!savepoints_.empty() && savepoints_.back().id == id) {
      savepoints_.pop_back();
...
boost::asio::awaitable<void>
transaction_participant_impl::prepare_commit(forge::db::core::participant_access&) {
   if (!staged_) {
      FORGE_THROW_EXCEPTION(exceptions::transaction_not_staged,
                            "authenticated transaction has no staged version");
   }
   co_return;
}
...
void transaction_participant_impl::set_staged(staged_version value) {
   if (staged_) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_version,
                            "authenticated transaction is already staged");
   }
