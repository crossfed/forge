...
inline forge::db::core::record_key data_key(std::string_view algorithm, std::span<const std::byte> digest) {
...
inline forge::db::core::record_key ref_key(std::string_view algorithm, std::span<const std::byte> digest, const owner_ref& owner) {
   auto key = std::vector<std::byte>{};
...
inline forge::db::core::record_key ref_prefix(std::string_view algorithm, std::span<const std::byte> digest) {
   auto key = std::vector<std::byte>{};
   key.reserve(algorithm.size() + digest.size() + 9U);
   key.push_back(static_cast<std::byte>(0x20U));
   append_blob_ref(key, algorithm, digest);
...
inline std::optional<encoded_ref> ref_from_data_key(const forge::db::core::record_key& key) {
