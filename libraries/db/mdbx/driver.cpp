...
void validate_config(const config& value) {
   ...
   if (value.max_readers == 0 || value.max_readers > static_cast<std::size_t>(std::numeric_limits<unsigned>::max())) {
   ...
   if (!ordered(value.map.lower_size, value.map.current_size) ||
       !ordered(value.map.current_size, value.map.upper_size) || !ordered(value.map.lower_size, value.map.upper_size)) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_config, "MDBX geometry must satisfy lower <= current <= upper");
   }
   ...
   if (value.map.page_size.has_value()) {
      ...
      if (page_size < static_cast<std::uint32_t>(mdbx_limits_pgsize_min()) ||
          page_size > static_cast<std::uint32_t>(mdbx_limits_pgsize_max()) || (page_size & (page_size - 1U)) != 0U) {
         FORGE_THROW_EXCEPTION(exceptions::invalid_config, "MDBX page size is unsupported",
      ...
      if (!aligned(value.map.lower_size) || !aligned(value.map.current_size) || !aligned(value.map.upper_size) ||
          !aligned(value.map.growth_step) || !aligned(value.map.shrink_threshold)) {
         FORGE_THROW_EXCEPTION(exceptions::invalid_config, "MDBX geometry must align to configured page size",
                               forge::exceptions::ctx("page-size", page_size));
      }
   }
}
...
driver::driver(std::shared_ptr<detail::driver_impl> impl) : impl_{std::move(impl)} {}

driver::~driver() = default;

boost::asio::awaitable<std::shared_ptr<driver>> driver::open(config value, forge::asio::affine::executor executor) {
   validate_config(value);
   co_return co_await open_managed(std::move(value), std::move(executor), {});
}

boost::asio::awaitable<std::shared_ptr<driver>> driver::open(config value,
                                                             forge::asio::affine::lane::options lane_options) {
   validate_config(value);
   auto lane = std::make_shared<forge::asio::affine::lane>(std::move(lane_options));
   auto executor = lane->get_executor();
   co_return co_await open_managed(std::move(value), std::move(executor), std::move(lane));
}

boost::asio::awaitable<std::shared_ptr<driver>> driver::open_managed(config value,
                                                                     forge::asio::affine::executor executor,
                                                                     std::shared_ptr<forge::asio::affine::lane> lane) {
   if (!executor.valid()) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_config, "MDBX requires a valid affine executor");
   }

   auto opened = co_await executor.execute({.name = "mdbx-open"}, [value = std::move(value)]() mutable {
      return std::pair{detail::environment::open(value), std::this_thread::get_id()};
   });
   auto impl = std::make_shared<detail::driver_impl>(std::move(executor), std::move(opened.first), opened.second,
                                                     std::move(lane));
