...
driver_impl::driver_impl(forge::asio::affine::executor executor, std::shared_ptr<environment> environment,
                         std::thread::id affine_thread, std::shared_ptr<forge::asio::affine::lane> lane_owner)
    : lane_owner_{std::move(lane_owner)}, executor_{std::move(executor)}, environment_{std::move(environment)},
      writer_gate_{std::make_shared<forge::asio::gate>()}, affine_thread_{affine_thread} {}
...
boost::asio::awaitable<std::unique_ptr<forge::db::core::session>> driver_impl::open_transaction() {
   auto ticket = co_await writer_gate_->acquire();
   auto* transaction = co_await executor_.execute({.name = "mdbx-begin-write"}, [environment = environment_]() {
      auto* transaction = static_cast<MDBX_txn*>(nullptr);
      require_mdbx_success(
          mdbx_txn_begin(environment->native(), nullptr, static_cast<MDBX_txn_flags_t>(MDBX_TXN_TRY), &transaction),
          "mdbx_txn_begin write");
      return transaction;
   });
   co_return std::make_unique<transaction_session>(shared_from_this(), std::move(ticket), transaction);
}

std::unique_ptr<forge::db::core::session> driver_impl::open_snapshot() {
   auto* anchor = static_cast<MDBX_txn*>(nullptr);
   require_mdbx_success(mdbx_txn_begin(environment_->native(), nullptr, MDBX_TXN_RDONLY, &anchor),
...
boost::asio::awaitable<void> driver_impl::create_checkpoint(std::string destination) {
   ...
   co_await executor_.execute(
       {.name = "mdbx-create-checkpoint"}, [environment = environment_, destination = std::move(destination)] {
          ...
          try {
             ...
             require_mdbx_success(mdbx_env_copy(environment->native(), data.c_str(), MDBX_CP_COMPACT),
...
boost::asio::awaitable<void> driver_impl::flush(bool sync) {
   ...
   auto ticket = co_await writer_gate_->acquire();
   require_open();
   co_await executor_.execute({.name = sync ? "mdbx-flush-sync" : "mdbx-flush-poll"},
...
boost::asio::awaitable<void> driver_impl::close() {
   ...
   co_await executor_.execute({.name = "mdbx-close"}, [environment = environment_] { environment->close(); });
...
void driver_impl::abort_sync(std::vector<MDBX_txn*> transactions) noexcept {
   run_sync("mdbx-dropped-rollback", [transactions = std::move(transactions)]() mutable {
      for (auto iterator = transactions.rbegin(); iterator != transactions.rend(); ++iterator) {
         if (*iterator != nullptr) {
...
void driver_impl::require_open() const {
   if (closed_.load(std::memory_order_acquire)) {
      FORGE_THROW_EXCEPTION(forge::db::core::exceptions::driver_closed, "MDBX driver is closed");
   }
}
...
void driver_impl::run_sync(std::string name, std::function<void()> operation) noexcept {
   ...
   try {
      ...
      auto future = boost::asio::co_spawn(
          context, executor_.execute({.name = std::move(name)}, [owned] { (*owned)(); }), boost::asio::use_future);
      context.run();
      future.get();
   } catch (...) {
      try {
