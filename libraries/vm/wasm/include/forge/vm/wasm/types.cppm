...
template <typename... Ts> constexpr std::size_t max_layout_size_v = std::max({sizeof(Ts)...});

template <typename... Ts> constexpr std::size_t max_alignof_v = std::max({alignof(Ts)...});
...
template <typename T, typename Alternative> constexpr std::size_t get_alternatives_index_v<T, Alternative> = 0;
...
template <typename Ret> struct dispatcher<false, Ret> {
   template <std::size_t I, typename Vis, typename Var> static constexpr Ret _case(Vis&&, Var&&) {
      throw exceptions::interpreter("variant visit shouldn't be here");
   }
   template <std::size_t I, typename Vis, typename Var> static constexpr Ret _switch(Vis&&, Var&&) {
      throw exceptions::interpreter("variant visit shouldn't be here");
   }
};

template <typename Ret> struct dispatcher<true, Ret> {
   template <std::size_t I, typename Vis, typename Var> static constexpr Ret _case(Vis&& vis, Var&& var) {
      return std::invoke(std::forward<Vis>(vis), std::forward<Var>(var).template get<I>());
   }

   template <std::size_t I, typename Vis, typename Var> static constexpr Ret _switch(Vis&& vis, Var&& var) {
      constexpr std::size_t sz = std::decay_t<Var>::variant_size();
      switch (var.index()) {
      case I + 0: {
         return dispatcher<I + 0 < sz, Ret>::template _case<I + 0>(std::forward<Vis>(vis), std::forward<Var>(var));
      }
      case I + 1: {
         return dispatcher<I + 1 < sz, Ret>::template _case<I + 1>(std::forward<Vis>(vis), std::forward<Var>(var));
      }
      case I + 2: {
         return dispatcher<I + 2 < sz, Ret>::template _case<I + 2>(std::forward<Vis>(vis), std::forward<Var>(var));
      }
      case I + 3: {
         return dispatcher<I + 3 < sz, Ret>::template _case<I + 3>(std::forward<Vis>(vis), std::forward<Var>(var));
      }
      default: {
         return dispatcher<I + 4 < sz, Ret>::template _switch<I + 4>(std::forward<Vis>(vis), std::forward<Var>(var));
      }
      }
   }
};
...
union variant_storage<T0, T1, T2, T3, T...> {
   V4 template <typename A> constexpr variant_storage(A&& arg) : _tail{std::forward<A>(arg)} {}
   variant_storage<T...> _tail;
};
template <typename T0> union variant_storage<T0> {
   V1
};
template <typename T0, typename T1> union variant_storage<T0, T1> {
   V2
};
template <typename T0, typename T1, typename T2> union variant_storage<T0, T1, T2> {
   V3
};
template <typename T0, typename T1, typename T2, typename T3> union variant_storage<T0, T1, T2, T3> {
   V4
};
...
template <int I, typename Storage> constexpr decltype(auto) variant_storage_get(Storage&& val) {
   if constexpr (I == 0) {
...
template <class Visitor, typename Variant> constexpr auto visit(Visitor&& vis, Variant&& var) {
...
template <typename... Alternatives> class variant {
 ...
 public:
   ...
   constexpr variant(T&& alt)
       : _which(detail::get_alternatives_index_v<std::decay_t<T>, Alternatives...>), _storage(std::forward<T>(alt)) {}
...
#pragma GCC diagnostic pop
   ...
   inline constexpr uint16_t index() const {
      return _which;
   }
   ...
   inline constexpr void toggle_exiting_which() {
      _which ^= 0x100;
   }
   inline constexpr void clear_exiting_which() {
      _which &= 0xFF;
   }
   inline constexpr void set_exiting_which() {
      _which |= 0x100;
   }
...
template <typename StackElem> inline void print_result(const std::optional<StackElem>& result) {
   if (result) {
      std::cout << "result: ";
...
struct maybe_void_t {
   template <typename T> inline constexpr friend T&& operator,(T&& val, maybe_void_t) {
...
namespace detail {
template <size_t N, size_t I, typename T, typename... Ts> struct subtuple_impl;

template <size_t N, size_t I, typename T, typename... Ts> struct subtuple_impl<N, I, std::tuple<T, Ts...>> {
   using type = decltype(std::tuple_cat(std::declval<std::tuple<T>>(),
                                        std::declval<typename subtuple_impl<N, I + 1, std::tuple<Ts...>>::type>()));
};

template <size_t N, typename T, typename... Ts> struct subtuple_impl<N, N, std::tuple<T, Ts...>> {
   using type = std::tuple<T>;
};

template <size_t N, typename T> using subtuple_t = typename subtuple_impl<N, 0, T>::type;

template <typename> struct generate_subtuples_impl;

template <typename T, typename... Ts> struct generate_subtuples_impl<std::tuple<T, Ts...>> {
   template <size_t... Is> static constexpr auto value(std::index_sequence<Is...>) {
      return std::make_tuple(std::declval<subtuple_t<Is, std::tuple<T, Ts...>>>()...);
   }
};

template <typename T>
using generate_subtuples_t =
    decltype(generate_subtuples_impl<T>::value(std::make_index_sequence<std::tuple_size_v<T>>{}));

template <typename T, typename... Ts> struct generate_all_subtuples_impl;

template <typename T, typename... Ts> struct generate_all_subtuples_impl<std::tuple<T, Ts...>> {
...
template <> struct generate_all_subtuples_impl<std::tuple<>> {
   using type = std::tuple<>;
};

template <typename Tuple> constexpr auto generate_all_subtuples(Tuple&& tup) {
   return std::declval<typename generate_all_subtuples_impl<Tuple>::type>();
}

template <size_t N> struct tuple_index {
   static constexpr size_t value = N;
};

template <size_t N, size_t I, typename Insert, typename Tuple> struct insert_type {
...
template <size_t I, typename Tuple, typename Indices> constexpr auto get_tuple_size_from_index() {
...
template <size_t I, typename Tuple> constexpr auto get_tuple_size() {
   if constexpr (I >= std::tuple_size_v<Tuple>)
      return -1;
   else
...
template <size_t N, size_t I, typename Tuple, typename Indices> struct index_inserter {
   static constexpr int32_t size_of_element = get_tuple_size<N, Tuple>();

   template <size_t M = N, size_t J = I> static constexpr auto value(std::enable_if_t<M != J, int> = 0) {
...
template <typename Tuple, typename Indices, size_t... Is> constexpr auto reorder_tuple(std::index_sequence<Is...>) {
...
using reorder_tuple_t =
    decltype(reorder_tuple<Tuple, Indices>(std::make_index_sequence<std::tuple_size_v<Indices> - 1>{}));

// sort the tuple by largest subtuple to smallest subtuple
template <size_t N, size_t I, typename Tuple, typename Indices> constexpr auto tuple_sort() {
   if constexpr (N == I)
...
template <size_t N, size_t I, typename Tuple> struct tuple_trim;
...
template <size_t N, typename Tuple> using tuple_trim_t = typename detail::tuple_trim<N, 0, Tuple>::type;
} // namespace forge::vm::wasm
...
template <typename T> using guarded_vector = managed_vector<T, growable_allocator>;
...
struct module {
   ...
   void make_jit_module() {
      ...
      if (auto types_size = types.size(); types_size > 0) {
         ...
         for (uint32_t i = 0; i < types_size; ++i) {
            ...
            jit_mod->types.emplace_back(
                jit_mod_t::jit_func_type{type.form,
      ...
      if (auto exports_size = exports.size(); exports_size > 0) {
         ...
         for (uint32_t i = 0; i < exports_size; ++i) {
            ...
            jit_mod->exports.emplace_back(jit_mod_t::jit_export_entry{
                {entry.field_str.data(), entry.field_str.data() + entry.field_str.size()}, entry.kind, entry.index});
         }
      }
      ...
      if (auto data_size = data.size(); data_size > 0) {
         ...
         for (uint32_t i = 0; i < data_size; ++i) {
            ...
            jit_mod->data.emplace_back(jit_mod_t::jit_data_segment{
                data_seg.index, data_seg.offset, {data_seg.data.data(), data_seg.data.data() + data_seg.data.size()}});
         }
      }
   ...
   template <typename Imports> static uint32_t get_imported_functions_size_impl(const Imports& imports) {
   ...
   inline opcode* get_function_pc(uint32_t fidx) const {
      detail::check<exceptions::interpreter>((fidx >= get_imported_functions_size()),
   ...
   inline uint32_t get_function_locals_size(uint32_t index) const {
      detail::check<exceptions::interpreter>((index >= get_imported_functions_size()),
...
template <typename ElemT, size_t ElemSz, typename Allocator = nullptr_t> class stack {
 public:
   ...
   template <typename Alloc = Allocator, typename = std::enable_if_t<!std::is_same_v<Alloc, nullptr_t>, int>>
   stack(uint32_t n, Alloc&& alloc) : _store(alloc, n) {}
 ...
 private:
   using base_data_store_t = std::conditional_t<std::is_same_v<Allocator, std::nullptr_t>, unmanaged_vector<ElemT>,
