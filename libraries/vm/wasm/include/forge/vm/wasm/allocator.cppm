   ...
   class growable_allocator {
    public:
      ...
      void reclaim(const T* ptr, size_t size=0) {
         ...
         detail::check<exceptions::allocation>((size == 0 || (char*)(ptr + size) == (_base + _offset)), "reclaiming memory must be strictly LIFO");
         if ( size != 0 )
      ...
      void finalize() {
         if(_capacity != _offset) {
            ...
            if (final_size < _capacity) { // final_size can grow to _capacity after align_to_page.
               ...
               detail::check<exceptions::allocation>((munmap(_base + final_size, _capacity - final_size) == 0), "failed to finalize growable_allocator");
      ...
      void free() { detail::fail<exceptions::allocation>( "unimplemented"); }
