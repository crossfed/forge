...
#define FORGE_VM_WASM_HAS_MEMBER_TY(TY, NAME)                         \
   forge::vm::wasm::detail::overloaded {                             \
    ...
    }(forge::vm::wasm::detail::wrapper_t<TY>{})
...
namespace forge::vm::wasm {
   ...
   template <typename F>
   constexpr bool is_callable(F&& fn) { return FORGE_VM_WASM_HAS_MEMBER(fn, operator()); }
...
#pragma GCC diagnostic pop
   ...
   namespace detail {
      template <typename T>
      constexpr inline std::true_type is_argument_proxy_type(argument_proxy<T>) { return {}; }
      template <typename T, std::size_t LA>
      constexpr inline std::true_type is_argument_proxy_type(argument_proxy<T, LA>) { return {}; }
      template <typename T>
      constexpr inline std::false_type is_argument_proxy_type(T) { return {}; }
   }

   template <typename T>
   inline constexpr bool is_argument_proxy_type_v = std::is_same_v<decltype(detail::is_argument_proxy_type(std::declval<T>())), std::true_type>;
...
export namespace forge::vm::wasm {
   ...
   struct type_converter : public running_context<Host, Execution_Interface> {
      ...
      FORGE_VM_WASM_FROM_WASM(bool, (uint32_t value)) { return value ? 1 : 0; }
   ...
   namespace detail {
      ...
      template <typename S, typename Type_Converter>
      inline constexpr bool has_to_wasm_v =
   ...
   struct registered_host_functions {
      ...
      static void resolve_impl(Module& mod) {
         ...
         for (std::size_t i = 0; i < mod.imports.size(); i++) {
            ...
            detail::check<exceptions::link>((current_mappings.named_mapping.count({ mod_name, fn_name })), std::string("no mapping for imported function ") + fn_name);
            imports[i] = current_mappings.named_mapping[{ mod_name, fn_name }];
            detail::check<exceptions::link>((mod.imports[i].kind == Function), std::string("importing non-function ") + fn_name);
            detail::check<exceptions::link>((current_mappings.host_functions[imports[i]] == mod.types[mod.imports[i].type.func_t]), std::string("wrong type for imported function ") + fn_name);
         }
      }
