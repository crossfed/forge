...
export import forge.vm.wasm.argument_proxy;
export import forge.vm.wasm.execution_interface;
...
struct registered_host_functions {
   ...
   template <auto Func, typename... Preconditions> static void add(const std::string& mod, const std::string& name) {
      using args = flatten_parameters_t<FORGE_VM_WASM_AUTO_PARAM(Func)>;
      using res = return_type_t<FORGE_VM_WASM_AUTO_PARAM(Func)>;
