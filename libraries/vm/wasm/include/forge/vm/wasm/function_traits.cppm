...
template <auto FN> inline constexpr static bool is_function_v = is_function(FORGE_VM_WASM_AUTO_PARAM(FN));

template <auto FN> inline constexpr static bool is_member_function_v = is_member_function(FORGE_VM_WASM_AUTO_PARAM(FN));
...
template <auto FN> using return_type_t = decltype(return_type(FORGE_VM_WASM_AUTO_PARAM(FN)));

template <typename F>
constexpr auto class_from_member(F&& fn) -> std::tuple_element_t<1, detail::get_types_t<false, F>>;

template <auto FN> using class_from_member_t = decltype(class_from_member(FORGE_VM_WASM_AUTO_PARAM(FN)));

template <typename F>
constexpr auto flatten_parameters(F&& fn) -> std::tuple_element_t<2, detail::get_types_t<false, F>>;

template <auto FN> using flatten_parameters_t = decltype(flatten_parameters(FORGE_VM_WASM_AUTO_PARAM(FN)));

template <typename F>
constexpr auto decayed_flatten_parameters(F&& fn) -> std::tuple_element_t<2, detail::get_types_t<true, F>>;

template <auto FN>
using decayed_flatten_parameters_t = decltype(decayed_flatten_parameters(FORGE_VM_WASM_AUTO_PARAM(FN)));

template <std::size_t N, typename F>
constexpr auto parameter_at(F&& fn) -> std::tuple_element_t<N, decltype(flatten_parameters(std::declval<F>()))>;

template <std::size_t N, auto FN> using parameter_at_t = decltype(parameter_at<N>(FORGE_VM_WASM_AUTO_PARAM(FN)));

template <std::size_t N, typename F> constexpr auto parameters_from(F&& fn) -> detail::parameters_from_impl_t<N, F>;

template <std::size_t N, auto FN> using parameters_from_t = decltype(parameters_from<N>(FORGE_VM_WASM_AUTO_PARAM(FN)));
