...
/* clang-format off */
#define FORGE_VM_WASM_INTERPRET_CONTROL_FLOW_OPS(opcode_macro)   \
...
#define FORGE_VM_WASM_INTERPRET_BR_TABLE_OP(opcode_macro)        \
   opcode_macro(br_table, 0x0E)
#define FORGE_VM_WASM_INTERPRET_RETURN_OP(opcode_macro)          \
   opcode_macro(return_, 0x0F)
#define FORGE_VM_WASM_INTERPRET_CALL_OPS(opcode_macro)           \
   opcode_macro(call, 0x10)                     \
   opcode_macro(call_indirect, 0x11)            \
   opcode_macro(br_table_data, 0x12)
#define FORGE_VM_WASM_INTERPRET_CALL_IMM_OPS(opcode_macro)       \
...
#define FORGE_VM_WASM_INTERPRET_PARAMETRIC_OPS(opcode_macro)     \
...
#define FORGE_VM_WASM_INTERPRET_VARIABLE_ACCESS_OPS(opcode_macro)\
...
#define FORGE_VM_WASM_INTERPRET_MEMORY_OPS(opcode_macro)         \
...
#define FORGE_VM_WASM_INTERPRET_I32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i32_const, 0x41)
#define FORGE_VM_WASM_INTERPRET_I64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(i64_const, 0x42)
#define FORGE_VM_WASM_INTERPRET_F32_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f32_const, 0x43)
#define FORGE_VM_WASM_INTERPRET_F64_CONSTANT_OPS(opcode_macro)   \
   opcode_macro(f64_const, 0x44)
#define FORGE_VM_WASM_INTERPRET_COMPARISON_OPS(opcode_macro)     \
...
#define FORGE_VM_WASM_INTERPRET_NUMERIC_OPS(opcode_macro)        \
...
#define FORGE_VM_WASM_INTERPRET_CONVERSION_OPS(opcode_macro)     \
...
#define FORGE_VM_WASM_INTERPRET_EXIT_OP(opcode_macro)            \
   opcode_macro(exit, 0xC0)
#define FORGE_VM_WASM_INTERPRET_EMPTY_OPS(opcode_macro)          \
...
#define FORGE_VM_WASM_INTERPRET_ERROR_OPS(opcode_macro)          \
   opcode_macro(error, 0xFF)

/* clang-format on */

#define FORGE_VM_WASM_INTERPRET_CREATE_ENUM(name, code) name = code,

#define FORGE_VM_WASM_INTERPRET_CREATE_STRINGS(name, code) #name,

#define FORGE_VM_WASM_INTERPRET_CREATE_MAP(name, code) {code, #name},

#define FORGE_VM_WASM_INTERPRET_OPCODE_NAME_if_
#define FORGE_VM_WASM_INTERPRET_OPCODE_NAME_else_
#define FORGE_VM_WASM_INTERPRET_OPCODE_NAME_return_

#define FORGE_VM_WASM_INTERPRET_OPCODE_NAME_TEST() 1
#define FORGE_VM_WASM_INTERPRET_OPCODE_NAME_TEST_FORGE_VM_WASM_INTERPRET_OPCODE_NAME_TEST 0,
#define FORGE_VM_WASM_INTERPRET_OPCODE_NAME_TEST_1 1, ignore
#define FORGE_VM_WASM_INTERPRET_EXPAND(x) x
#define FORGE_VM_WASM_INTERPRET_CAT2(x, y) x##y
#define FORGE_VM_WASM_INTERPRET_CAT(x, y) FORGE_VM_WASM_INTERPRET_CAT2(x, y)
#define FORGE_VM_WASM_INTERPRET_APPLY(f, args) f args
#define FORGE_VM_WASM_INTERPRET_FIX_OPCODE_NAME_0(name) name##_t
#define FORGE_VM_WASM_INTERPRET_FIX_OPCODE_NAME_1(name) name##t
#define FORGE_VM_WASM_INTERPRET_FIX_OPCODE_NAME(iskeyword, garbage) FORGE_VM_WASM_INTERPRET_FIX_OPCODE_NAME_##iskeyword

#define FORGE_VM_WASM_INTERPRET_OPCODE_T(name)                                                                         \
   FORGE_VM_WASM_INTERPRET_APPLY(                                                                                      \
       FORGE_VM_WASM_INTERPRET_FIX_OPCODE_NAME,                                                                        \
       (FORGE_VM_WASM_INTERPRET_CAT(                                                                                   \
           FORGE_VM_WASM_INTERPRET_OPCODE_NAME_TEST_,                                                                  \
           FORGE_VM_WASM_INTERPRET_EXPAND(                                                                             \
               FORGE_VM_WASM_INTERPRET_OPCODE_NAME_TEST FORGE_VM_WASM_INTERPRET_OPCODE_NAME_##name()))))(name)

#define FORGE_VM_WASM_INTERPRET_CREATE_EXIT_TYPE(name, code)                                                           \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      uint32_t pc;                                                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define FORGE_VM_WASM_INTERPRET_CREATE_CONTROL_FLOW_TYPES(name, code)                                                  \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)() {}                                                                      \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(uint32_t data) : data(data) {}                                            \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(uint32_t d, uint32_t pc, uint16_t i, uint16_t oi)                         \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_BR_TABLE_TYPE(name, code)                                                       \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      struct elem_t {                                                                                                  \
         uint32_t pc;                                                                                                  \
         uint32_t stack_pop;                                                                                           \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_TYPES(name, code)                                                               \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define FORGE_VM_WASM_INTERPRET_CREATE_CALL_TYPES(name, code)                                                          \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define FORGE_VM_WASM_INTERPRET_CREATE_CALL_IMM_TYPES(name, code)                                                      \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      uint32_t index;                                                                                                  \
      uint16_t locals;                                                                                                 \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_VARIABLE_ACCESS_TYPES(name, code)                                               \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      uint32_t index;                                                                                                  \
      static constexpr uint8_t opcode = code;                                                                          \
   };

#define FORGE_VM_WASM_INTERPRET_CREATE_MEMORY_TYPES(name, code)                                                        \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_I32_CONSTANT_TYPE(name, code)                                                   \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)() = default;                                                              \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(uint32_t n) {                                                    \
         data.ui = n;                                                                                                  \
      }                                                                                                                \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(int32_t n) {                                                     \
         data.i = n;                                                                                                   \
      }                                                                                                                \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_I64_CONSTANT_TYPE(name, code)                                                   \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)() = default;                                                              \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(uint64_t n) {                                                    \
         data.ui = n;                                                                                                  \
      }                                                                                                                \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(int64_t n) {                                                     \
         data.i = n;                                                                                                   \
      }                                                                                                                \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_F32_CONSTANT_TYPE(name, code)                                                   \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)() = default;                                                              \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(uint32_t n) {                                                    \
         data.ui = n;                                                                                                  \
      }                                                                                                                \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(float n) {                                                       \
         data.f = n;                                                                                                   \
      }                                                                                                                \
      union {                                                                                                          \
         uint32_t ui;                                                                                                  \
...
#define FORGE_VM_WASM_INTERPRET_CREATE_F64_CONSTANT_TYPE(name, code)                                                   \
   struct FORGE_VM_WASM_INTERPRET_OPCODE_T(name) {                                                                     \
      FORGE_VM_WASM_INTERPRET_OPCODE_T(name)() = default;                                                              \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(uint64_t n) {                                                    \
         data.ui = n;                                                                                                  \
      }                                                                                                                \
      explicit FORGE_VM_WASM_INTERPRET_OPCODE_T(name)(double n) {                                                      \
         data.f = n;                                                                                                   \
      }                                                                                                                \
      union {                                                                                                          \
         uint64_t ui;                                                                                                  \
...
#define FORGE_VM_WASM_INTERPRET_IDENTITY(name, code) forge::vm::wasm::interpret::FORGE_VM_WASM_INTERPRET_OPCODE_T(name),
#define FORGE_VM_WASM_INTERPRET_IDENTITY_END(name, code)                                                               \
   forge::vm::wasm::interpret::FORGE_VM_WASM_INTERPRET_OPCODE_T(name)
