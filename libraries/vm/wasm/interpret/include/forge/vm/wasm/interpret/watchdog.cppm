...
export module forge.vm.wasm.interpret.watchdog;
...
class watchdog {
 ...
 public:
   ...
   template <typename TimeUnits> explicit watchdog(const TimeUnits& duration) : _duration(duration) {}
   ...
   class guard {
    public:
      ...
      guard(const TimeUnits& duration, F&& callback)
          : _callback(std::forward<F>(callback)), _run_state(running), _duration(duration),
...
class null_watchdog {
 public:
   template <typename F> null_watchdog scoped_run(F&&) {
      return *this;
   }
