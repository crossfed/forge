...
export module forge.vm.wasm.interpret.host_function:function_traits;
...
template <typename F> constexpr bool is_callable(F&& fn) {
   return FORGE_VM_WASM_INTERPRET_HAS_MEMBER(fn, operator());
}
...
template <auto FN> using return_type_t = decltype(return_type(FORGE_VM_WASM_INTERPRET_AUTO_PARAM(FN)));

template <typename F>
constexpr auto class_from_member(F&& fn) -> std::tuple_element_t<1, detail::get_types_t<false, F>>;

template <auto FN> using class_from_member_t = decltype(class_from_member(FORGE_VM_WASM_INTERPRET_AUTO_PARAM(FN)));

template <typename F>
constexpr auto flatten_parameters(F&& fn) -> std::tuple_element_t<2, detail::get_types_t<false, F>>;

template <auto FN> using flatten_parameters_t = decltype(flatten_parameters(FORGE_VM_WASM_INTERPRET_AUTO_PARAM(FN)));

template <typename F>
constexpr auto decayed_flatten_parameters(F&& fn) -> std::tuple_element_t<2, detail::get_types_t<true, F>>;

template <auto FN>
using decayed_flatten_parameters_t = decltype(decayed_flatten_parameters(FORGE_VM_WASM_INTERPRET_AUTO_PARAM(FN)));
