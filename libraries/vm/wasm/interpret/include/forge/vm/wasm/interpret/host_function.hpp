...
#define FORGE_VM_WASM_INTERPRET_HAS_MEMBER(ARG, NAME)                                                                  \
   forge::vm::wasm::interpret::detail::overloaded{                                                                     \
       [](auto&& f,                                                                                                    \
          std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>>&& forge::vm::wasm::interpret::detail::pass_type< \
                               decltype(&std::decay_t<decltype(f)>::type::NAME)>(),                                    \
...
#define FORGE_VM_WASM_INTERPRET_HAS_TEMPLATE_MEMBER(ARG, NAME)                                                         \
   forge::vm::wasm::interpret::detail::overloaded{                                                                     \
       [&](auto&& f,                                                                                                   \
           std::enable_if_t<std::is_class_v<std::decay_t<decltype(f)>>&& forge::vm::wasm::interpret::detail::          \
                                pass_type<decltype(&std::decay_t<decltype(f)>::type::template NAME)>(),                \
                            int> = 0) constexpr { return true; },                                                      \
       [](...) constexpr { return false; }}(forge::vm::wasm::interpret::detail::wrapper_t<decltype(ARG)>{})
...
#define FORGE_VM_WASM_INTERPRET_FROM_WASM(TYPE, PARAMS)                                                                \
   template <typename T>                                                                                               \
   auto from_wasm FORGE_VM_WASM_INTERPRET_FROM_WASM_ADD_TAG                                                            \
       PARAMS const->std::enable_if_t<std::is_same_v<T, TYPE>, TYPE>

#define FORGE_VM_WASM_INTERPRET_INVOKE_ON(TYPE, CONDITION)                                                             \
   ::forge::vm::wasm::interpret::invoke_on<false, TYPE>(CONDITION, args...)
