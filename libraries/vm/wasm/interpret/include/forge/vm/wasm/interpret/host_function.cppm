...
export module forge.vm.wasm.interpret.host_function;

export import forge.vm.wasm.interpret.argument_proxy;
export import forge.vm.wasm.interpret.execution_interface;
export import forge.vm.wasm.interpret.types;
...
struct type_converter : public running_context<Host, Execution_Interface> {
   ...
   FORGE_VM_WASM_INTERPRET_FROM_WASM(bool, (uint32_t value)) {
      return value ? 1 : 0;
   }
...
inline constexpr bool has_from_wasm_v = FORGE_VM_WASM_INTERPRET_HAS_TEMPLATE_MEMBER_TY(Type_Converter, from_wasm<S>);
...
struct registered_host_functions {
   ...
   template <auto Func, typename... Preconditions> static void add(const std::string& mod, const std::string& name) {
      using args = flatten_parameters_t<FORGE_VM_WASM_INTERPRET_AUTO_PARAM(Func)>;
      using res = return_type_t<FORGE_VM_WASM_INTERPRET_AUTO_PARAM(Func)>;
