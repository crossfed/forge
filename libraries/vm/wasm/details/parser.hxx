...
namespace forge::vm::wasm {
   ...
   struct max_mutable_globals_checker<Options, std::void_t<max_mutable_globals_t<Options>>> {
      ...
      void on_mutable_global(const Options& options, uint8_t type) {
         ...
         _counter += size;
         FORGE_VM_WASM_ASSERT(_counter <= options.max_mutable_global_bytes && _counter >= size, exceptions::parse, "mutable globals exceeded limit");
      }
...
#define MAX_ELEMENTS(name, default_)\
   ...
   struct max_func_local_bytes_checker<Options, std::void_t<decltype(std::declval<Options>().max_func_local_bytes)>> {
      ...
      void on_type(const Options& options, std::uint8_t type) {
         unsigned size = get_size_for_type(type);
         _count += size;
         FORGE_VM_WASM_ASSERT(_count <= options.max_func_local_bytes && _count >= size, exceptions::parse, "local variable limit exceeded");
      }
   ...
   struct max_control_depth_checker<Options, std::void_t<decltype(std::declval<Options>().max_control_depth)>> {
      void on_control(const Options& options) {
         ++_count;
         FORGE_VM_WASM_ASSERT(_count <= options.max_control_depth, exceptions::parse, "Nested depth exceeded");
      }
    ...
    public:
      ...
      int validate_utf8_code_point(wasm_code_ptr& code) {
         ...
         if (ch < 0x80) {
           ...
           FORGE_VM_WASM_ASSERT((ch & 0xC0) == 0xC0, exceptions::parse, "invalid utf8 encoding");
            unsigned char b2 = *code++;
            FORGE_VM_WASM_ASSERT((b2 & 0xC0) == 0x80, exceptions::parse, "invalid utf8 encoding");
            uint32_t code_point =
              (static_cast<uint32_t>(ch - 0xC0u) << 6u) +
              (static_cast<uint32_t>(b2 - 0x80u));
            FORGE_VM_WASM_ASSERT(0x80 <= code_point && code_point < 0x800, exceptions::parse, "invalid utf8 encoding");
            return 2;
         } else if(ch < 0xF0) {
            unsigned char b2 = *code++;
            FORGE_VM_WASM_ASSERT((b2 & 0xC0) == 0x80, exceptions::parse, "invalid utf8 encoding");
            unsigned char b3 = *code++;
            FORGE_VM_WASM_ASSERT((b3 & 0xC0) == 0x80, exceptions::parse, "invalid utf8 encoding");
            uint32_t code_point =
              (static_cast<uint32_t>(ch - 0xE0u) << 12u) +
              (static_cast<uint32_t>(b2 - 0x80u) << 6u) +
              (static_cast<uint32_t>(b3 - 0x80u));
            FORGE_VM_WASM_ASSERT((0x800 <= code_point && code_point < 0xD800) ||
                          (0xE000 <= code_point && code_point < 0x10000),
                          exceptions::parse, "invalid utf8 encoding");
            return 3;
         } else if (ch < 0xF8) {
            unsigned char b2 = *code++;
            FORGE_VM_WASM_ASSERT((b2 & 0xC0) == 0x80, exceptions::parse, "invalid utf8 encoding");
            ...
            FORGE_VM_WASM_ASSERT((b4 & 0xC0) == 0x80, exceptions::parse, "invalid utf8 encoding");
            uint32_t code_point =
            ...
            FORGE_VM_WASM_ASSERT((0x10000 <= code_point && code_point < 0x110000),
                          exceptions::parse, "invalid utf8 encoding");
            return 4;
         }
         FORGE_VM_WASM_ASSERT(false, exceptions::parse, "invalid utf8 encoding");
      }
      ...
      guarded_vector<uint8_t> parse_utf8_string(wasm_code_ptr& code, std::uint32_t max_size) {
         ...
         FORGE_VM_WASM_ASSERT(len <= max_size, exceptions::parse, "name too long");
      ...
      void parse_module(wasm_code_ptr& code_ptr, size_t sz, module& mod, DebugInfo& debug) {
         _mod = &mod;
         FORGE_VM_WASM_ASSERT(parse_magic(code_ptr) == constants::magic, exceptions::parse, "magic number did not match");
         FORGE_VM_WASM_ASSERT(parse_version(code_ptr) == constants::version, exceptions::parse,
         ...
         for (;;) {
            ...
            FORGE_VM_WASM_ASSERT(id == 0 || id > highest_section_id, exceptions::parse, "section out of order");
            ...
            switch (id) {
               ...
               default: FORGE_VM_WASM_ASSERT(false, exceptions::parse, "error invalid section id");
            }
         }
         FORGE_VM_WASM_ASSERT(_mod->code.size() == _mod->functions.size(), exceptions::parse, "code section must have the same size as the function section" );
      ...
      inline void parse_name_section(wasm_code_ptr& code) {
         ...
         if(code.bounds() == code.offset()) return;
         FORGE_VM_WASM_ASSERT(false, exceptions::parse, "Invalid subsection Id");
      }
      ...
      void parse_import_entry(wasm_code_ptr& code, import_entry& entry) {
         ...
         switch ((uint8_t)entry.kind) {
            case external_kind::Function:
               ...
               FORGE_VM_WASM_ASSERT(type < _mod->types.size(), exceptions::parse, "Invalid function type");
               break;
            default: FORGE_VM_WASM_ASSERT(false, exceptions::unsupported_import, "only function imports are supported");
         }
      }
      ...
      uint8_t parse_flags(wasm_code_ptr& code) {
         if (detail::get_allow_u32_limits_flags(_options)) {
            ...
            FORGE_VM_WASM_ASSERT(*code == 0x0 || *code == 0x1, exceptions::parse, "invalid flags");
            return *code++;
         }
      }
      ...
      void parse_table_type(wasm_code_ptr& code, table_type& tt) {
         ...
         FORGE_VM_WASM_ASSERT(tt.element_type == types::anyfunc, exceptions::parse, "table must have type anyfunc");
         tt.limits.flags   = parse_flags(code);
         tt.limits.initial = parse_varuint32(code);
         if (tt.limits.flags) {
            tt.limits.maximum = parse_varuint32(code);
            FORGE_VM_WASM_ASSERT(tt.limits.initial <= tt.limits.maximum, exceptions::parse, "table max size less than min size");
         }
         FORGE_VM_WASM_ASSERT(tt.limits.initial <= detail::get_max_table_elements(_options), exceptions::parse, "table size exceeds limit");
      ...
      void parse_global_variable(wasm_code_ptr& code, global_variable& gv) {
         ...
         FORGE_VM_WASM_ASSERT(ct == types::i32 || ct == types::i64 || ct == types::f32 || ct == types::f64,
                       exceptions::parse, "invalid global content type");
      ...
      void parse_memory_type(wasm_code_ptr& code, memory_type& mt) {
         ...
         FORGE_VM_WASM_ASSERT(mt.limits.initial <= detail::get_max_pages(_options), exceptions::parse, "initial memory out of range");
         // WASM specification
         FORGE_VM_WASM_ASSERT(mt.limits.initial <= 65536u, exceptions::parse, "initial memory out of range");
         if (mt.limits.flags) {
            mt.limits.maximum = parse_varuint32(code);
            FORGE_VM_WASM_ASSERT(mt.limits.maximum >= mt.limits.initial, exceptions::parse, "maximum must be at least minimum");
            FORGE_VM_WASM_ASSERT(mt.limits.maximum <= 65536u, exceptions::parse, "maximum memory out of range");
         }
      }
      ...
      void parse_export_entry(wasm_code_ptr& code, export_entry& entry) {
         ...
         switch(entry.kind) {
            case external_kind::Function: FORGE_VM_WASM_ASSERT(entry.index < _mod->get_functions_total(), exceptions::parse, "function export out of range"); break;
            case external_kind::Table: FORGE_VM_WASM_ASSERT(entry.index < _mod->tables.size(), exceptions::parse, "table export out of range"); break;
            case external_kind::Memory: FORGE_VM_WASM_ASSERT(entry.index < _mod->memories.size(), exceptions::parse, "memory export out of range"); break;
            case external_kind::Global:
               FORGE_VM_WASM_ASSERT(entry.index < _mod->globals.size(), exceptions::parse, "global export out of range");
               FORGE_VM_WASM_ASSERT(!detail::get_forbid_export_mutable_globals(_options) || !_mod->globals.at(entry.index).type.mutability,
                             exceptions::parse, "cannot export mutable globals");
               break;
            default: FORGE_VM_WASM_ASSERT(false, exceptions::parse, "Unknown export kind"); break;
         }
      }

      void parse_func_type(wasm_code_ptr& code, func_type& ft) {
         ft.form                              = *code++;
         FORGE_VM_WASM_ASSERT(ft.form == 0x60, exceptions::parse, "invalid function type");
         decltype(ft.param_types) param_types = { _allocator, parse_varuint32(code) };
         for (size_t i = 0; i < param_types.size(); i++) {
            uint8_t pt        = *code++;
            param_types.at(i) = pt;
            FORGE_VM_WASM_ASSERT(pt == types::i32 || pt == types::i64 || pt == types::f32 || pt == types::f64,
                          exceptions::parse, "invalid function param type");
         }
         ft.param_types  = std::move(param_types);
         ft.return_count = *code++;
         FORGE_VM_WASM_ASSERT(ft.return_count < 2, exceptions::parse, "invalid function return count");
         if (ft.return_count > 0) {
            uint8_t rt        = *code++;
            ft.return_type = rt;
            FORGE_VM_WASM_ASSERT(rt == types::i32 || rt == types::i64 || rt == types::f32 || rt == types::f64,
                          exceptions::parse, "invalid function return type");
         }
      }
      ...
      void parse_elem_segment(wasm_code_ptr& code, elem_segment& es) {
         ...
         FORGE_VM_WASM_ASSERT(tt != nullptr, exceptions::parse, "table not declared");
         es.index = parse_varuint32(code);
         FORGE_VM_WASM_ASSERT(es.index == 0, exceptions::parse, "only table index of 0 is supported");
         parse_init_expr(code, es.offset, types::i32);
         uint32_t           size  = parse_varuint32(code);
         FORGE_VM_WASM_ASSERT(size <= detail::get_max_element_segment_elements(_options), exceptions::parse, "elem segment too large");
         decltype(es.elems) elems = { _allocator, size };
         for (uint32_t i = 0; i < size; i++) {
            uint32_t index                     = parse_varuint32(code);
            elems.at(i)                        = index;
            FORGE_VM_WASM_ASSERT(index < _mod->get_functions_total(), exceptions::parse,  "elem for undefined function");
         }
      ...
      void parse_init_expr(wasm_code_ptr& code, init_expr& ie, uint8_t type) {
         ...
         switch (ie.opcode) {
            case opcodes::i32_const:
               ...
               FORGE_VM_WASM_ASSERT(type == types::i32, exceptions::parse, "expected i32 initializer");
               break;
            case opcodes::i64_const:
               ie.value.i64 = parse_varint64(code);
               FORGE_VM_WASM_ASSERT(type == types::i64, exceptions::parse, "expected i64 initializer");
               break;
            case opcodes::f32_const:
               ie.value.f32 = parse_raw<uint32_t>(code);
               FORGE_VM_WASM_ASSERT(type == types::f32, exceptions::parse, "expected f32 initializer");
               break;
            case opcodes::f64_const:
               ie.value.f64 = parse_raw<uint64_t>(code);
               FORGE_VM_WASM_ASSERT(type == types::f64, exceptions::parse, "expected f64 initializer");
               break;
            default:
               FORGE_VM_WASM_ASSERT(false, exceptions::parse,
                             "initializer expression can only acception i32.const, i64.const, f32.const and f64.const");
         }
         FORGE_VM_WASM_ASSERT((*code++) == opcodes::end, exceptions::parse, "no end op found");
      }

      void parse_function_body(wasm_code_ptr& code, function_body& fb, std::size_t idx) {
         fb.size   = parse_varuint32(code);
         FORGE_VM_WASM_ASSERT(fb.size <= detail::get_max_code_bytes(_options), exceptions::parse, "Function body too large");
         const auto&         before    = code.offset();
         const auto&         local_cnt = parse_varuint32(code);
         _current_function_index++;
         FORGE_VM_WASM_ASSERT(local_cnt <= detail::get_max_local_sets(_options), exceptions::parse, "Number of local sets exceeds limit");
         ...
         for (size_t i = 0; i < local_cnt; i++) {
            ...
            FORGE_VM_WASM_ASSERT(type == types::i32 || type == types::i64 || type == types::f32 || type == types::f64,
                          exceptions::parse, "invalid local type");
         ...
         code += fb.size-1;
         FORGE_VM_WASM_ASSERT(detail::get_allow_code_after_function_end(_options) || *code == 0x0B,
                       exceptions::parse, "failed parsing function body, expected 'end'");
         ++code;
      }
      ...
      struct operand_stack_type_tracker {
         ...
         void push(uint8_t type) {
            ...
            FORGE_VM_WASM_ASSERT(operand_depth < std::numeric_limits<uint32_t>::max(), exceptions::parse, "integer overflow in operand depth");
            ++operand_depth;
         ...
         void pop(uint8_t expected) {
            ...
            FORGE_VM_WASM_ASSERT(!state.empty(), exceptions::parse, "unexpected pop");
            if (state.back() != unreachable_tag) {
               FORGE_VM_WASM_ASSERT(state.back() == expected || state.back() == any_type, exceptions::parse, "wrong type");
         ...
         uint8_t pop() {
            FORGE_VM_WASM_ASSERT(!state.empty() && state.back() != scope_tag, exceptions::parse, "unexpected pop");
            if (state.back() == unreachable_tag)
               return any_type;
            else {
         ...
         void pop_scope(uint8_t expected_result = types::pseudo) {
            pop(expected_result);
            FORGE_VM_WASM_ASSERT(!state.empty(), exceptions::parse, "unexpected end");
            if (state.back() == unreachable_tag) {
               local_bytes_checker.pop_unreachable();
               state.pop_back();
            }
            FORGE_VM_WASM_ASSERT(state.back() == scope_tag, exceptions::parse, "unexpected end");
            state.pop_back();
         ...
         void finish() {
            ...
            FORGE_VM_WASM_ASSERT(state.empty(), exceptions::parse, "stack not empty at scope end");
         }
      ...
      struct local_types_t {
         local_types_t(const func_type& ft, const guarded_vector<local_entry>& locals_arg) :
            ...
            for (uint32_t i = 0; i < locals_arg.size(); ++i) {
               ...
               FORGE_VM_WASM_ASSERT (count <= 0xFFFFFFFFu - locals_arg[i].count, exceptions::parse, "too many locals");
               count += locals_arg[i].count;
               _boundaries.push_back(count);
            }
         }
         uint8_t operator[](uint32_t local_idx) const {
            FORGE_VM_WASM_ASSERT(local_idx < _boundaries.back(), exceptions::parse, "undefined local");
      ...
      void parse_function_body_code(wasm_code_ptr& code, size_t bounds, const detail::max_func_local_bytes_stack_checker<Options>& local_bytes_checker,
         ...
         auto handle_branch_target = [&](uint32_t label, branch_t address) {
            FORGE_VM_WASM_ASSERT(label < pc_stack.size(), exceptions::parse, "invalid label");
         ...
         auto exit_scope = [&]() {
            ...
            FORGE_VM_WASM_ASSERT(pc_stack.size(), exceptions::parse, "unexpected end instruction");
            // an if with an empty else cannot have a return value
            FORGE_VM_WASM_ASSERT(!pc_stack.back().is_if || pc_stack.back().expected_result == types::pseudo, exceptions::parse, "wrong type");
         ...
         auto check_in_bounds = [&]{
            FORGE_VM_WASM_ASSERT(!detail::get_allow_code_after_function_end(_options) || !pc_stack.empty(),
                          exceptions::parse, "code after function end");
         };

         while (code.offset() < bounds) {
            FORGE_VM_WASM_ASSERT(pc_stack.size() <= detail::get_max_nested_structures(_options), exceptions::parse,
            ...
            switch (*code++) {
               ...
               case opcodes::end: {
                  check_in_bounds();
                  exit_scope();
                  FORGE_VM_WASM_ASSERT(detail::get_allow_code_after_function_end(_options) ||
                                !pc_stack.empty() || code.offset() == bounds, exceptions::parse, "function too short");
               ...
               case opcodes::block: {
                  ...
                  FORGE_VM_WASM_ASSERT(expected_result == types::i32 || expected_result == types::i64 ||
                                expected_result == types::f32 || expected_result == types::f64 ||
                                expected_result == types::pseudo, exceptions::parse,
               ...
               case opcodes::else_: {
                  ...
                  FORGE_VM_WASM_ASSERT(old_index.is_if, exceptions::parse, "else outside if");
               ...
               case opcodes::br_table: {
                  ...
                  FORGE_VM_WASM_ASSERT(table_size <= detail::get_max_br_table_elements(_options), exceptions::parse, "Too many labels in br_table");
                  ...
                  for (size_t i = 0; i < table_size; i++) {
                     ...
                     if(i == 0) {
                        ...
                        FORGE_VM_WASM_ASSERT(result_type == one_result, exceptions::parse, "br_table labels must have the same type");
                     }
                  }
                  uint32_t label = parse_varuint32(code);
                  auto branch = handler.emit_default(compute_depth_change(label));
                  handle_branch_target(label, branch);
                  FORGE_VM_WASM_ASSERT(table_size == 0 || result_type == pc_stack[pc_stack.size() - label - 1].label_result,
                                exceptions::parse, "br_table labels must have the same type");
                  op_stack.start_unreachable();
               } break;
               case opcodes::call: {
                  check_in_bounds();
                  ...
                  FORGE_VM_WASM_ASSERT(ft.return_count <= 1, exceptions::parse, "unsupported");
                  if(ft.return_count)
               ...
               case opcodes::call_indirect: {
                  ...
                  FORGE_VM_WASM_ASSERT(_mod->tables.size() > 0, exceptions::parse, "call_indirect requires a table");
                  op_stack.pop(types::i32);
                  for(uint32_t i = 0; i < ft.param_types.size(); ++i)
                     op_stack.pop(ft.param_types[ft.param_types.size() - i - 1]);
                  FORGE_VM_WASM_ASSERT(ft.return_count <= 1, exceptions::parse, "unsupported");
                  ...
                  FORGE_VM_WASM_ASSERT(*code == 0, exceptions::parse, "call_indirect must end with 0x00.");
                  code++; // 0x00
                  break;
               }
               ...
               case opcodes::select: {
                  ...
                  FORGE_VM_WASM_ASSERT(t0 == t1 || t0 == any_type || t1 == any_type, exceptions::parse, "incorrect types for select");
               ...
               case opcodes::set_global: {
                  check_in_bounds();
                  uint32_t global_idx = parse_varuint32(code);
                  FORGE_VM_WASM_ASSERT(_mod->globals.at(global_idx).type.mutability, exceptions::parse, "cannot set const global");
...
#define LOAD_OP(op_name, max_align, type)                            \
               case opcodes::op_name: {                              \
                  check_in_bounds();                                 \
                  FORGE_VM_WASM_ASSERT(_mod->memories.size() > 0, exceptions::parse, "load requires memory"); \
                  uint32_t alignment = parse_varuint32(code);        \
                  uint32_t offset = parse_varuint32(code);           \
                  FORGE_VM_WASM_ASSERT(alignment <= uint32_t(max_align), exceptions::parse, "alignment cannot be greater than size."); \
                  FORGE_VM_WASM_ASSERT(offset <= detail::get_max_memory_offset(_options), exceptions::parse, "load offset too large."); \
                  op_stack.pop(types::i32);                          \
...
#define STORE_OP(op_name, max_align, type)                           \
               case opcodes::op_name: {                              \
                  check_in_bounds();                                 \
                  FORGE_VM_WASM_ASSERT(_mod->memories.size() > 0, exceptions::parse, "store requires memory"); \
                  uint32_t alignment = parse_varuint32(code);        \
                  uint32_t offset = parse_varuint32(code);           \
                  FORGE_VM_WASM_ASSERT(alignment <= uint32_t(max_align), exceptions::parse, "alignment cannot be greater than size."); \
                  FORGE_VM_WASM_ASSERT(offset <= detail::get_max_memory_offset(_options), exceptions::parse, "store offset too large."); \
...
#undef STORE_OP
               ...
               case opcodes::current_memory:
                  FORGE_VM_WASM_ASSERT(_mod->memories.size() != 0, exceptions::parse, "memory.size requires memory");
                  op_stack.push(types::i32);
                  FORGE_VM_WASM_ASSERT(*code == 0, exceptions::parse, "memory.size must end with 0x00");
                  code++;
                  code_writer.emit_current_memory();
                  break;
               case opcodes::grow_memory:
                  check_in_bounds();
                  FORGE_VM_WASM_ASSERT(_mod->memories.size() != 0, exceptions::parse, "memory.grow requires memory");
                  op_stack.pop(types::i32);
                  op_stack.push(types::i32);
                  FORGE_VM_WASM_ASSERT(*code == 0, exceptions::parse, "memory.grow must end with 0x00");
                  code++;
...
#undef UNOP
#undef BINOP
               default: FORGE_VM_WASM_ASSERT(false, exceptions::parse, "Illegal instruction");
            }
         }
         FORGE_VM_WASM_ASSERT( pc_stack.empty(), exceptions::parse, "function body too long" );
      ...
      void parse_data_segment(wasm_code_ptr& code, data_segment& ds) {
         FORGE_VM_WASM_ASSERT(_mod->memories.size() != 0, exceptions::parse, "data requires memory");
         ds.index = parse_varuint32(code);
         parse_init_expr(code, ds.offset, types::i32);
         auto len =  parse_varuint32(code);
         FORGE_VM_WASM_ASSERT(len <= detail::get_max_data_segment_bytes(_options), exceptions::parse, "data segment too large.");
         FORGE_VM_WASM_ASSERT(static_cast<uint64_t>(static_cast<uint32_t>(ds.offset.value.i32)) + len <= detail::get_max_linear_memory_init(_options),
                       exceptions::parse, "out-of-bounds data section");
         auto guard = code.scoped_shrink_bounds(len);
      ...
      inline void parse_section_impl(wasm_code_ptr& code, vec<Elem>& elems, std::uint32_t max_elements, ParseFunc&& elem_parse) {
         auto count = parse_varuint32(code);
         FORGE_VM_WASM_ASSERT(count <= max_elements, exceptions::parse, "number of section elements exceeded limit");
      ...
      inline void parse_section(wasm_code_ptr&                                                                 code,
         ...
         parse_section_impl(code, elems, 1, [&](wasm_code_ptr& code, memory_type& mt, std::size_t idx) {
            FORGE_VM_WASM_ASSERT(idx == 0, exceptions::parse, "only one memory is permitted");
      ...
      inline void parse_section(wasm_code_ptr&                                                        code,
         ...
         FORGE_VM_WASM_ASSERT(ft.return_count == 0 && ft.param_types.size() == 0, exceptions::parse, "wrong type for start");
      }
      template <uint8_t id>
      inline void
      parse_section(wasm_code_ptr&                                                                   code,
      ...
      inline void parse_section(wasm_code_ptr&                                                                 code,
         ...
         FORGE_VM_WASM_ASSERT( elems.size() == _mod->functions.size(), exceptions::parse, "code section must have the same size as the function section" );
      ...
      void validate_exports() const {
         ...
         FORGE_VM_WASM_ASSERT(it == export_names.end(), exceptions::parse, "duplicate export name");
      }
