...
FORGE_VM_WASM_EXPORT namespace forge::vm::wasm {
   ...
   struct guarded_ptr {
      ...
      inline guarded_ptr& operator+=(size_t i) {
         FORGE_VM_WASM_ASSERT(i <= static_cast<std::size_t>(bnds - raw_ptr), exceptions::pointer_out_of_bounds, "overbounding pointer");
         raw_ptr += i;
         return *this;
      }

      inline guarded_ptr& operator++() {
         FORGE_VM_WASM_ASSERT(raw_ptr < bnds, exceptions::pointer_out_of_bounds, "overbounding pointer");
         raw_ptr += 1;
         return *this;
      }
      ...
      inline T& operator* () const {
         FORGE_VM_WASM_ASSERT(raw_ptr < bnds, exceptions::pointer_out_of_bounds, "accessing out of bounds");
         return *raw_ptr;
      }
      ...
      auto scoped_shrink_bounds(std::size_t n) {
         FORGE_VM_WASM_ASSERT(n <= static_cast<std::size_t>(bnds - raw_ptr), exceptions::pointer_out_of_bounds, "guarded ptr out of bounds");
         T* old_bnds = bnds;
         bnds = raw_ptr + n;
      ...
      auto scoped_consume_items(std::size_t n) {
         ...
         return scope_guard{ [this, old_bnds, exceptions, x](){
            FORGE_VM_WASM_ASSERT(exceptions != std::uncaught_exceptions() || raw_ptr == bnds, exceptions::pointer_out_of_bounds, "guarded_ptr not advanced");
            bnds = old_bnds;
         } };
      }
      ...
      inline T at(size_t index) const {
         FORGE_VM_WASM_ASSERT(index < static_cast<std::size_t>(bnds - raw_ptr), exceptions::pointer_out_of_bounds, "accessing out of bounds");
