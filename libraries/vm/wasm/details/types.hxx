...
FORGE_VM_WASM_EXPORT namespace forge::vm::wasm {
   ...
   struct module {
      ...
      static uint32_t get_imported_functions_size_impl(const Imports& imports) {
         ...
         // `FORGE_VM_WASM_ASSERT(i < _size)`. The check is unnecessary since we iterate from `0` to `_size`.
      ...
      inline opcode* get_function_pc( uint32_t fidx ) const {
         FORGE_VM_WASM_ASSERT( fidx >= get_imported_functions_size(), exceptions::interpreter, "trying to get the PC of an imported function" );
      ...
      inline uint32_t get_function_locals_size(uint32_t index) const {
         FORGE_VM_WASM_ASSERT(index >= get_imported_functions_size(), exceptions::interpreter, "imported functions do not have locals");
