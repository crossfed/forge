...
struct profile_manager {
   profile_manager() {
      ...
      FORGE_VM_WASM_ASSERT(res == 0, exceptions::profile, "Failed to start timer");
   ...
   void start(profile_data* data) {
      FORGE_VM_WASM_ASSERT(!current_data, exceptions::profile, "Already profiling in the current thread");
      current_data = data;
   }
   void stop() {
