...
namespace forge::vm::wasm {
   ...
   struct null_host_functions {
      ...
      void operator()(A&&...) const {
         FORGE_VM_WASM_ASSERT(false, exceptions::interpreter,
   ...
   class execution_context_base {
    ...
    public:
      ...
      inline void initialize_globals_impl(const Module& mod) {
         FORGE_VM_WASM_ASSERT(_globals.empty(), exceptions::memory, "initialize_globals called on non-empty _globals");
      ...
      inline void reset(Module& mod) {
         FORGE_VM_WASM_ASSERT(_mod->error == nullptr, exceptions::interpreter, _mod->error);
         ...
         if(mod.memories.size()) {
            FORGE_VM_WASM_ASSERT(mod.memories[0].limits.initial <= _max_pages, exceptions::allocation, "Cannot allocate initial linear memory.");
         ...
         for (uint32_t i = 0; i < mod.data.size(); i++) {
            ...
            FORGE_VM_WASM_ASSERT(required_memory <= available_memory, exceptions::memory, "data out of range");
    ...
    protected:
      ...
      static void type_check_args(const Func_type& ft, Args&&...) {
         FORGE_VM_WASM_ASSERT(sizeof...(Args) == ft.param_types.size(), exceptions::interpreter, "wrong number of arguments");
         uint32_t i = 0;
         FORGE_VM_WASM_ASSERT((... && (to_wasm_type_v<detail::type_converter_t<Host>, Args> == ft.param_types.at(i++))), exceptions::interpreter, "unexpected argument type");
      }
   ...
   class execution_context : public execution_context_base<execution_context<Host>, Host, false> {
    ...
    public:
      ...
      inline operand_stack_elem  get_global(uint32_t index) {
         FORGE_VM_WASM_ASSERT(index < _mod->globals.size(), exceptions::interpreter, "global index out of range");
         FORGE_VM_WASM_ASSERT(index < _globals.size(), exceptions::interpreter, "index for _globals out of range in get_global for interpreter");
      ...
      inline void set_global(uint32_t index, const operand_stack_elem& el) {
         FORGE_VM_WASM_ASSERT(index < _mod->globals.size(), exceptions::interpreter, "global index out of range");
         FORGE_VM_WASM_ASSERT(index < _globals.size(), exceptions::interpreter, "index for _globals out of range");
         auto& gl = _mod->globals[index];
         FORGE_VM_WASM_ASSERT(gl.type.mutability, exceptions::interpreter, "global is not mutable");
         visit(overloaded{ [&](const i32_const_t& i) {
                                  FORGE_VM_WASM_ASSERT(gl.type.content_type == types::i32, exceptions::interpreter,
                                                "expected i32 global type");
                                  _globals[index].value.i32 = i.data.ui;
                               },
                                [&](const i64_const_t& i) {
                                   FORGE_VM_WASM_ASSERT(gl.type.content_type == types::i64, exceptions::interpreter,
                                                 "expected i64 global type");
                                   _globals[index].value.i64 = i.data.ui;
                                },
                                [&](const f32_const_t& f) {
                                   FORGE_VM_WASM_ASSERT(gl.type.content_type == types::f32, exceptions::interpreter,
                                                 "expected f32 global type");
                                   _globals[index].value.f32 = f.data.ui;
                                },
                                [&](const f64_const_t& f) {
                                   FORGE_VM_WASM_ASSERT(gl.type.content_type == types::f64, exceptions::interpreter,
      ...
      inline void type_check(const func_type& ft) {
         for (uint32_t i = 0; i < ft.param_types.size(); i++) {
            ...
            visit(overloaded{ [&](const i32_const_t&) {
                                     FORGE_VM_WASM_ASSERT(ft.param_types[i] == types::i32, exceptions::interpreter,
                                                   "function param type mismatch");
                                  },
                                   [&](const f32_const_t&) {
                                      FORGE_VM_WASM_ASSERT(ft.param_types[i] == types::f32, exceptions::interpreter,
                                                    "function param type mismatch");
                                   },
                                   [&](const i64_const_t&) {
                                      FORGE_VM_WASM_ASSERT(ft.param_types[i] == types::i64, exceptions::interpreter,
                                                    "function param type mismatch");
                                   },
                                   [&](const f64_const_t&) {
                                      FORGE_VM_WASM_ASSERT(ft.param_types[i] == types::f64, exceptions::interpreter,
                                                    "function param type mismatch");
                                   },
      ...
      inline std::optional<operand_stack_elem> execute(host_type* host, Visitor&& visitor, uint32_t func_index, Args&&... args) {
         FORGE_VM_WASM_ASSERT(func_index < std::numeric_limits<uint32_t>::max(), exceptions::interpreter,
