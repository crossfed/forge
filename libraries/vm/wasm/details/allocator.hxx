...
FORGE_VM_WASM_EXPORT namespace forge::vm::wasm {
   class bounded_allocator {
    public:
      ...
      T* alloc(size_t size = 1) {
         FORGE_VM_WASM_ASSERT((sizeof(T) * size) + index <= mem_size, exceptions::allocation, "wasm failed to allocate native");
      ...
      void free() {
         FORGE_VM_WASM_ASSERT(index > 0, exceptions::double_free, "double free");
         index = 0;
      }
      void                       reset() { index = 0; }
      size_t                     mem_size;
   ...
   class contiguous_allocator {
      public:
         ...
         contiguous_allocator(size_t size) {
            ...
            FORGE_VM_WASM_ASSERT(_base != MAP_FAILED, exceptions::allocation, "mmap failed.");
         }
         ...
         T* alloc(size_t size = 0) {
            ...
            if (aligned > _size) {
               ...
               FORGE_VM_WASM_ASSERT(new_base != MAP_FAILED, exceptions::allocation, "mmap failed.");
   ...
   private:
      ...
      blocks_by_size_t::iterator allocate_segment(std::size_t min_size) {
         ...
         FORGE_VM_WASM_ASSERT(base != MAP_FAILED, exceptions::allocation, "failed to allocate jit segment");
         segment s{base, size};
   ...
   class growable_allocator {
    public:
      ...
      explicit growable_allocator(size_t size) {
         FORGE_VM_WASM_ASSERT(size <= max_memory_size, exceptions::allocation, "Too large initial memory size");
         use_default_memory();
      }

      void use_default_memory() {
         FORGE_VM_WASM_ASSERT(_base == nullptr, exceptions::allocation, "default memory already allocated");
         ...
         FORGE_VM_WASM_ASSERT(_base != MAP_FAILED, exceptions::allocation, "failed to mmap for default memory.");
         _capacity = max_memory_size;
      }

      // size in bytes
      void use_fixed_memory(size_t size) {
         FORGE_VM_WASM_ASSERT(0 < size && size <= max_memory_size, exceptions::allocation, "Too large or 0 fixed memory size");
         FORGE_VM_WASM_ASSERT(_base == nullptr, exceptions::allocation, "Fixed memory already allocated");

         _base = (char*)mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
         FORGE_VM_WASM_ASSERT(_base != MAP_FAILED, exceptions::allocation, "mmap in use_fixed_memory failed.");
         _capacity = size;
      }
      ...
      T* alloc(size_t size = 0) {
         ...
         FORGE_VM_WASM_ASSERT(_capacity % alignof(T) == 0, exceptions::allocation, "alignment must divide _capacity.");
         _offset = align_offset<alignof(T)>(_offset);
         ...
         FORGE_VM_WASM_ASSERT ((_capacity - _offset) / sizeof(T) >= size, exceptions::allocation, "Allocated too much memory");
         size_t aligned = (sizeof(T) * size) + _offset;
         FORGE_VM_WASM_ASSERT (aligned <= _capacity, exceptions::allocation, "Allocated too much memory after aligned");
      ...
      void end_code(void * code_base) {
         ...
         if constexpr (IsJit) {
            ...
            FORGE_VM_WASM_ASSERT(err == 0, exceptions::allocation, "mprotect failed");
      ...
      void reclaim(const T* ptr, size_t size=0) {
         FORGE_VM_WASM_ASSERT( _offset / sizeof(T) >= size, exceptions::allocation, "reclaimed too much memory" );
         FORGE_VM_WASM_ASSERT( size == 0 || (char*)(ptr + size) == (_base + _offset), exceptions::allocation, "reclaiming memory must be strictly LIFO");
         if ( size != 0 )
      ...
      void finalize() {
         if(_capacity != _offset) {
            ...
            if (final_size < _capacity) { // final_size can grow to _capacity after align_to_page.
               ...
               FORGE_VM_WASM_ASSERT(munmap(_base + final_size, _capacity - final_size) == 0, exceptions::allocation, "failed to finalize growable_allocator");
      ...
      void free() { FORGE_VM_WASM_ASSERT(false, exceptions::allocation, "unimplemented"); }

      void release_base_memory()
      {
         if (_base != nullptr) {
            FORGE_VM_WASM_ASSERT(munmap(_base, _capacity) == 0, exceptions::allocation, "failed to release base memory");
            _base = nullptr;
         }
      }
   ...
   class fixed_stack_allocator {
    ...
    public:
      ...
      fixed_stack_allocator(size_t max_size) : max_size(max_size) {
         ...
         FORGE_VM_WASM_ASSERT( raw != MAP_FAILED, exceptions::allocation, "mmap failed to alloca pages" );
   ...
   class wasm_allocator {
    ...
    public:
      ...
      void alloc(size_t size = 1 /*in pages*/) {
         if (size == 0) return;
         FORGE_VM_WASM_ASSERT(page >= 0, exceptions::allocation, "require memory to allocate");
         FORGE_VM_WASM_ASSERT(size + page <= max_pages, exceptions::allocation, "exceeded max number of pages");
      ...
      void free(std::size_t size) {
         if (size == 0) return;
         FORGE_VM_WASM_ASSERT(page >= 0, exceptions::allocation, "require memory to deallocate");
         FORGE_VM_WASM_ASSERT(size <= static_cast<uint32_t>(page), exceptions::allocation, "freed too many pages");
         page -= size;
