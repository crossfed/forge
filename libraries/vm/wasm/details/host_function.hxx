   ...
   struct registered_host_functions {
      ...
      static void resolve_impl(Module& mod) {
         ...
         for (std::size_t i = 0; i < mod.imports.size(); i++) {
            ...
            FORGE_VM_WASM_ASSERT(current_mappings.named_mapping.count({ mod_name, fn_name }), exceptions::link,
                          std::string("no mapping for imported function ") + fn_name);
            imports[i] = current_mappings.named_mapping[{ mod_name, fn_name }];
            FORGE_VM_WASM_ASSERT(mod.imports[i].kind == Function, exceptions::link, std::string("importing non-function ") + fn_name);
            FORGE_VM_WASM_ASSERT(current_mappings.host_functions[imports[i]] == mod.types[mod.imports[i].type.func_t], exceptions::link, std::string("wrong type for imported function ") + fn_name);
         }
      }
