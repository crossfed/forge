...
#pragma GCC unroll 5
         ...
         inline constexpr void from( guarded_ptr<uint8_t>& code ) {
            uint8_t cnt = 0;
            for (;; cnt++) {
               FORGE_VM_WASM_ASSERT( cnt < bytes_needed<N>(), exceptions::interpreter, "incorrect leb128 encoding" );
               FORGE_VM_WASM_ASSERT( code.offset()+cnt < code.bounds(), exceptions::interpreter, "pointer out of bounds" );
               ...
               if ((storage[cnt] & 0x80) == 0) {
                  if( static_cast<size_t>(cnt + 1)  == bytes_needed<N>() ) {
                     ...
                     FORGE_VM_WASM_ASSERT((mask & storage[cnt]) == 0, exceptions::parse, "unused bits of unsigned leb128 must be 0");
                  }
                  break;
               }
            }
               ...
               if ((storage[cnt] & 0x80) == 0) {
                  if( static_cast<size_t>(cnt + 1) == bytes_needed<N>() ) {
                    uint32_t offset = N - 7*(bytes_needed<N>()-1);
                     ...
                     FORGE_VM_WASM_ASSERT((mask & storage[cnt]) == expected, exceptions::parse, "unused bits of signed leb128 must be the same as the sign bit");
                  }
                  break;
               }
            }
