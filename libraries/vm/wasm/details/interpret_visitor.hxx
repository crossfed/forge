...
#include "config.hxx"
#include "base_visitor.hxx"
#include "softfloat.hxx"
...
namespace forge::vm::wasm {
   ...
   struct interpret_visitor : base_visitor {
      ...
      [[gnu::always_inline]] inline void operator()(const call_indirect_t& op) {
         ...
         FORGE_VM_WASM_ASSERT(actual_type == expected_type, exceptions::interpreter, "bad call_indirect type");
         context.call(fn);
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i32();
         auto&       lhs = context.peek_operand().to_i32();
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i32.div_s divide by zero");
         FORGE_VM_WASM_ASSERT(!(lhs == std::numeric_limits<int32_t>::min() && rhs == -1), exceptions::interpreter,
      ...
      [[gnu::always_inline]] inline void operator()(const i32_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui32();
         auto&       lhs = context.peek_operand().to_ui32();
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i32.div_u divide by zero");
         lhs /= rhs;
      }
      [[gnu::always_inline]] inline void operator()(const i32_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i32();
         auto&       lhs = context.peek_operand().to_i32();
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i32.rem_s divide by zero");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_rem_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui32();
         auto&       lhs = context.peek_operand().to_ui32();
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i32.rem_u divide by zero");
         lhs %= rhs;
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_s_t& op) {
         ...
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i64.div_s divide by zero");
         FORGE_VM_WASM_ASSERT(!(lhs == std::numeric_limits<int64_t>::min() && rhs == -1), exceptions::interpreter,
      ...
      [[gnu::always_inline]] inline void operator()(const i64_div_u_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_ui64();
         auto&       lhs = context.peek_operand().to_ui64();
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i64.div_u divide by zero");
         lhs /= rhs;
      }
      [[gnu::always_inline]] inline void operator()(const i64_rem_s_t& op) {
         ...
         const auto& rhs = context.pop_operand().to_i64();
         auto&       lhs = context.peek_operand().to_i64();
         FORGE_VM_WASM_ASSERT(rhs != 0, exceptions::interpreter, "i64.rem_s divide by zero");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_s_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            FORGE_VM_WASM_ASSERT(!((af >= 2147483648.0f) || (af < -2147483648.0f)), exceptions::interpreter, "Error, f32.trunc_s/i32 overflow" );
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f32.trunc_s/i32 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_u_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            float af = oper.to_f32();
            FORGE_VM_WASM_ASSERT(!((af >= 4294967296.0f) || (af <= -1.0f)),exceptions::interpreter, "Error, f32.trunc_u/i32 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f32.trunc_u/i32 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i32_trunc_s_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            FORGE_VM_WASM_ASSERT(!((af >= 2147483648.0) || (af < -2147483648.0)), exceptions::interpreter, "Error, f64.trunc_s/i32 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f64.trunc_s/i32 unrepresentable");
            oper = i32_const_t{ static_cast<int32_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const i32_trunc_u_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            double af = oper.to_f64();
            FORGE_VM_WASM_ASSERT(!((af >= 4294967296.0) || (af <= -1.0)), exceptions::interpreter, "Error, f64.trunc_u/i32 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f64.trunc_u/i32 unrepresentable");
            oper = i32_const_t{ static_cast<uint32_t>(af) };
         }
      }
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_s_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            float af = oper.to_f32();
            FORGE_VM_WASM_ASSERT(!((af >= 9223372036854775808.0f) || (af < -9223372036854775808.0f)), exceptions::interpreter, "Error, f32.trunc_s/i64 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f32.trunc_s/i64 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_u_f32_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            float af = oper.to_f32();
            FORGE_VM_WASM_ASSERT(!((af >= 18446744073709551616.0f) || (af <= -1.0f)), exceptions::interpreter, "Error, f32.trunc_u/i64 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f32.trunc_u/i64 unrepresentable");
      ...
      [[gnu::always_inline]] inline void operator()(const i64_trunc_s_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            double af = oper.to_f64();
            FORGE_VM_WASM_ASSERT(!((af >= 9223372036854775808.0) || (af < -9223372036854775808.0)), exceptions::interpreter, "Error, f64.trunc_s/i64 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f64.trunc_s/i64 unrepresentable");
            oper = i64_const_t{ static_cast<int64_t>(af) };
         }
      }
      [[gnu::always_inline]] inline void operator()(const i64_trunc_u_f64_t& op) {
         ...
         if constexpr (use_softfloat) {
            ...
            double af = oper.to_f64();
            FORGE_VM_WASM_ASSERT(!((af >= 18446744073709551616.0) || (af <= -1.0)), exceptions::interpreter, "Error, f64.trunc_u/i64 overflow");
            FORGE_VM_WASM_ASSERT(!__builtin_isnan(af), exceptions::interpreter, "Error, f64.trunc_u/i64 unrepresentable");
            oper = i64_const_t{ static_cast<uint64_t>(af) };
         }
      }
