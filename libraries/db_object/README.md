...
`forge::db::driver`, so DB Object can run over in-memory test drivers, RocksDB
...
`forge::db::object::store` receives a shared `forge::db::driver` and a DB Object
record family:
...
DB Object can join an existing `forge::db::transaction`. This is how DB Object and
...
- `forge.db.object.record`: compatibility re-exports of public record value
  types from `forge.db.record`.
- `forge.db.object.cursor`: cursor/page request aliases and validation.
