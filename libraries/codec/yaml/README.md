...
`forge_codec_yaml` is the YAML codec boundary. Its API mirrors `forge_codec_json` so callers can
...
Target: `forge_codec_yaml`.

Dependencies: `forge_config_core`, `forge_schema`, `forge_variant`; the parser backend is
private.
...
auto parsed = forge::codec::yaml::read_document(
   "http:\n"
   "  bind-host: 127.0.0.1\n"
   "  bind-port: 8080\n");
...
auto config = forge::codec::yaml::read<http_config>(
   "bind-port: 9090\n"
   "tls-enabled: false\n");
if (!config.ok()) {
...
auto safe = forge::config::core::redact(document, registry);
auto output = forge::codec::yaml::write_document(safe);
if (!output.ok()) {
...
auto value = forge::codec::yaml::read_value("items:\n  - alpha\n  - beta\n");
if (!value.ok()) {
   report_diagnostics(value.diagnostics);
} else {
   auto compact = forge::codec::yaml::write_value(value.value, {.flow_style = true});
   if (!compact.ok()) {
...
- Do not preserve comments or formatting assumptions in v1; this is a codec, not
  a YAML editor.
- Do not put YAML-specific policy into `forge_config_core`.
