...
export module forge.api.http.parameters;
...
template <typename Stream> Stream& operator<<(Stream& stream, const body_stream&) {
   FORGE_THROW_EXCEPTION(forge::api::core::exceptions::protocol_error, "HTTP body streams are HTTP-only and cannot use generic binary serialization");
}
...
template <typename Stream> Stream& operator<<(Stream& stream, const upload_file&) {
   FORGE_THROW_EXCEPTION(forge::api::core::exceptions::protocol_error, "HTTP upload files are HTTP-only and cannot use generic binary serialization");
}
...
template <typename Stream> Stream& operator<<(Stream& stream, const stream_response&) {
   FORGE_THROW_EXCEPTION(forge::api::core::exceptions::protocol_error, "HTTP stream responses are HTTP-only and cannot use generic binary serialization");
}
