...
runtime. The unchanged EOSIO header corpus, `multi_index` and database
intrinsics belong to the next compatibility block.

## ABI, Tables And Ricardian Text

`forge-abigen` recognizes action, table and synchronous-call annotations, user
...
`forge-contract-check`; it is not accepted merely because JSON syntax is valid.
...
manifest consumed by `forge-contract-check`. Validation rejects unknown or
...
```bash
cmake -S sdk/contract -B build/contract-sdk -G Ninja \
