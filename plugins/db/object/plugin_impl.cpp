...
void plugin::impl::configure(config value) {
   ...
   if (state == phase::started || state == phase::stopping || state == phase::stopped) {
      FORGE_THROW_EXCEPTION(exceptions::stopped, "db object plugin cannot be configured after startup or stop");
   }
...
void plugin::impl::reject_started_setup() const {
   const auto state = current.load();
   if (state == phase::started || state == phase::stopping || state == phase::stopped) {
      FORGE_THROW_EXCEPTION(exceptions::stopped, "db object stores can only be added before startup");
   }
}

void plugin::impl::reject_duplicate_name(const std::string& name) const {
   if (stores.contains(name)) {
      FORGE_THROW_EXCEPTION(exceptions::duplicate_store, "db object store name is already registered",
...
void plugin::impl::add_store(std::string name,
   ...
   if (!driver) {
      FORGE_THROW_EXCEPTION(exceptions::invalid_argument, "db object driver must not be null",
                            forge::exceptions::ctx("store", name));
   }
...
void plugin::impl::start() {
   ...
   auto pending = std::vector<pending_open>{};
   {
      ...
      if (state == phase::stopping || state == phase::stopped) {
         FORGE_THROW_EXCEPTION(exceptions::stopped, "db object plugin is stopping");
      }

      for (const auto& item : settings.stores) {
         const auto found = stores.find(item.name);
         if (found == stores.end()) {
            FORGE_THROW_EXCEPTION(exceptions::startup_failed, "db object configured store is not registered",
...

   {
      ...
      for (auto& [name, record] : stores) {
         if (!record->driver) {
            FORGE_THROW_EXCEPTION(exceptions::startup_failed, "db object store has no driver",
                                  forge::exceptions::ctx("store", name));
         }
...
std::shared_ptr<managed_store> plugin::impl::require_store(const std::string& name) const {
   ...
   if (!record) {
      FORGE_THROW_EXCEPTION(exceptions::unknown_store, "db object store is not registered",
                            forge::exceptions::ctx("store", name));
   }
   return record;
}
...
plugin::impl::opened_store plugin::impl::require_open_store(const std::string& name) const {
   ...
   if ((state != phase::started && state != phase::stopping) || record->store == nullptr || record->driver == nullptr ||
       !record->started) {
      FORGE_THROW_EXCEPTION(exceptions::stopped, "db object store is not started",
                            forge::exceptions::ctx("store", name));
   }
...
std::optional<forge::config::component_descriptor> lifecycle::describe_config(const std::shared_ptr<plugin::impl>&) {
   return forge::config::describe_component<config>("plugins.db.object");
}
...
boost::asio::awaitable<void> lifecycle::startup(const std::shared_ptr<plugin::impl>& impl) {
   try {
      ...
      FORGE_THROW_EXCEPTION(exceptions::startup_failed, "db object plugin startup failed",
...
#else
      FORGE_THROW_EXCEPTION(exceptions::invalid_config, "db object rocksdb driver is not available in this build",
                            forge::exceptions::ctx("store", value.name));
#endif
   }

   FORGE_THROW_EXCEPTION(exceptions::invalid_config, "db object store driver is unsupported",
                         forge::exceptions::ctx("store", value.name),
