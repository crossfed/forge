...
forge::net::p2p::diagnostics::snapshot plugin::diagnostics_api::snapshot() const {
   return impl_->snapshot();
}

forge::net::p2p::diagnostics::snapshot plugin::diagnostics_api::snapshot(forge::net::p2p::diagnostics::options options) const {
   return impl_->require_source().snapshot(options);
}

forge::net::p2p::diagnostics::network_state plugin::diagnostics_api::network() const {
   return impl_->snapshot().network;
}

forge::net::p2p::resource_manager::snapshot plugin::diagnostics_api::resources() const {
   return impl_->snapshot().resources;
}

forge::net::p2p::pubsub::snapshot plugin::diagnostics_api::pubsub() const {
   return impl_->snapshot().pubsub;
}

std::vector<forge::net::p2p::diagnostics::peer> plugin::diagnostics_api::peers(filter value) const {
   return filter_peers(impl_->snapshot(), value);
}

forge::net::p2p::diagnostics::peer plugin::diagnostics_api::peer(forge::net::p2p::peer_id value) const {
