...
struct asset {
   static constexpr std::int64_t max_amount = forge::chain::protocol::asset::max_amount;
   ...
   asset(std::int64_t raw_amount, ::eosio::symbol raw_symbol)
       : asset(forge::chain::protocol::asset{raw_amount, raw_symbol}) {}

   constexpr asset(const forge::chain::protocol::asset& value) : amount(value.amount), symbol(value.sym) {}
   ...
   template <typename Stream> friend void raw_pack(Stream& stream, const asset& value) {
      forge::chain::protocol::raw_pack(stream, value.protocol_value());
   }

   template <typename Stream> friend void raw_unpack(Stream& stream, asset& value) {
      auto converted = forge::chain::protocol::asset{};
      forge::chain::protocol::raw_unpack(stream, converted);
 ...
 private:
   ...
   [[nodiscard]] constexpr forge::chain::protocol::asset protocol_value() const noexcept {
      auto value = forge::chain::protocol::asset{};
      value.amount = amount;
      value.sym = symbol;
      return value;
   }

   constexpr void assign(const forge::chain::protocol::asset& value) noexcept {
      amount = value.amount;
      symbol = value.sym;
   }
};
...
struct extended_asset {
   ...
   extended_asset(std::int64_t amount, extended_symbol symbol)
       : extended_asset(forge::chain::protocol::extended_asset{amount, symbol}) {}

   constexpr extended_asset(asset value, name raw_contract) : quantity(value), contract(raw_contract) {}

   constexpr extended_asset(const forge::chain::protocol::extended_asset& value)
   ...
   friend std::int64_t operator/(const extended_asset& left, const extended_asset& right) {
      if (left.contract != right.contract) {
         forge::chain::protocol::detail::fail_value("type mismatch");
      }
   ...
   template <typename Stream> friend void raw_unpack(Stream& stream, extended_asset& value) {
      auto converted = forge::chain::protocol::extended_asset{};
      forge::chain::protocol::raw_unpack(stream, converted);
      value.assign(converted);
   }

 private:
   [[nodiscard]] constexpr forge::chain::protocol::extended_asset protocol_value() const noexcept {
      return {quantity.protocol_value(), static_cast<const forge::chain::protocol::name&>(contract)};
   }

   constexpr void assign(const forge::chain::protocol::extended_asset& value) noexcept {
