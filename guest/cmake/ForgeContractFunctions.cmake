...
function(forge_install_contract_library)
   ...
   _forge_contract_target_dependencies("${_target}" _dependencies _dependency_scopes)
...
function(_forge_contract_unwrap_dependency_items target property dependencies_output link_only_output)
   get_target_property(_dependencies "${target}" "${property}")
   if(NOT _dependencies)
      set(_dependencies)
   endif()
   set(_unwrapped)
   set(_in_directory_scope FALSE)
   set(_directory_scope_entries 0)
   foreach(_dependency IN LISTS _dependencies)
      if(_dependency MATCHES "^::@\\([^)]+\\)$")
         if(_in_directory_scope)
            message(FATAL_ERROR "nested CMake directory-scope wrapper in contract library dependencies")
         endif()
         set(_in_directory_scope TRUE)
         set(_directory_scope_entries 0)
      elseif(_dependency STREQUAL "::@")
         if(NOT _in_directory_scope)
            message(FATAL_ERROR "unmatched CMake directory-scope wrapper in contract library dependencies")
         endif()
         if(_directory_scope_entries EQUAL 0)
            message(FATAL_ERROR "CMake directory-scope wrapper must contain at least one contract library dependency")
         endif()
         set(_in_directory_scope FALSE)
      elseif(_dependency MATCHES "^::@")
         message(FATAL_ERROR "malformed CMake directory-scope wrapper in contract library dependencies")
      else()
         if(_in_directory_scope)
            math(EXPR _directory_scope_entries "${_directory_scope_entries} + 1")
         endif()
         list(APPEND _unwrapped "${_dependency}")
      endif()
   endforeach()
   if(_in_directory_scope)
      message(FATAL_ERROR "unterminated CMake directory-scope wrapper in contract library dependencies")
   endif()

   set(_normalized)
   set(_link_only)
   foreach(_dependency IN LISTS _unwrapped)
      ...
      list(APPEND _link_only "${_is_link_only}")
   endforeach()
   set(${dependencies_output} "${_normalized}" PARENT_SCOPE)
   set(${link_only_output} "${_link_only}" PARENT_SCOPE)
endfunction()

function(_forge_contract_target_dependencies target dependencies_output scopes_output)
   _forge_contract_unwrap_dependency_items(
      "${target}" INTERFACE_LINK_LIBRARIES _dependencies _link_only
   )
   get_target_property(_imported "${target}" IMPORTED)
   if(NOT _imported)
      _forge_contract_unwrap_dependency_items(
         "${target}" LINK_LIBRARIES _implementation_dependencies _implementation_link_only
      )
      if(TRUE IN_LIST _implementation_link_only)
         message(FATAL_ERROR "LINK_ONLY is not supported in contract library LINK_LIBRARIES")
      endif()
   endif()

   set(_scoped_dependencies)
   set(_scopes)
   ...
   if(_dependency_count GREATER 0)
      ...
      foreach(_index RANGE 0 ${_dependency_last})
         list(GET _dependencies ${_index} _dependency)
         list(GET _link_only ${_index} _is_link_only)
         if(_is_link_only)
            if(NOT _imported AND NOT _dependency IN_LIST _implementation_dependencies)
               message(FATAL_ERROR "LINK_ONLY contract library dependency is not linked by its owner: ${_dependency}")
            endif()
            set(_scope PRIVATE)
         else()
            if(NOT _imported AND NOT _dependency IN_LIST _implementation_dependencies)
               message(
                  FATAL_ERROR
                  "INTERFACE-only contract library dependencies are not supported; "
                  "declare ${_dependency} through forge_add_contract_library(LIBRARIES ...) "
                  "or target_link_libraries(... PUBLIC ...)"
               )
            endif()
            set(_scope PUBLIC)
         endif()
         list(FIND _scoped_dependencies "${_dependency}" _existing_index)
         if(_existing_index EQUAL -1)
            list(APPEND _scoped_dependencies "${_dependency}")
            list(APPEND _scopes "${_scope}")
         else()
            list(GET _scopes ${_existing_index} _existing_scope)
            if(NOT _existing_scope STREQUAL _scope)
               message(
                  FATAL_ERROR
                  "contract library dependency has conflicting PUBLIC and PRIVATE scopes: ${_dependency}"
               )
            endif()
         endif()
      endforeach()
   ...
   if(NOT _imported)
      foreach(_dependency IN LISTS _implementation_dependencies)
         if(NOT _dependency IN_LIST _scoped_dependencies)
            message(
               FATAL_ERROR
               "contract library implementation dependency is absent from its static link interface: ${_dependency}"
            )
         endif()
      endforeach()
   endif()
   set(${dependencies_output} "${_scoped_dependencies}" PARENT_SCOPE)
   set(${scopes_output} "${_scopes}" PARENT_SCOPE)
endfunction()
...
function(_forge_contract_write_graph target libraries output)
   ...
   file(WRITE "${_path}" "set(FORGE_CONTRACT_GRAPH_VERSION 2)\n")
   ...
   foreach(_node IN LISTS _nodes)
      ...
      set(_dependency_ids)
      set(_public_dependency_ids)
      set(_private_dependency_ids)
      set(_guest_dependencies)
      set(_public_guest_dependencies)
      set(_private_guest_dependencies)
      _forge_contract_target_dependencies("${_node}" _dependencies _dependency_scopes)
      list(LENGTH _dependencies _dependency_count)
      if(_dependency_count GREATER 0)
         math(EXPR _dependency_last "${_dependency_count} - 1")
         foreach(_dependency_index RANGE 0 ${_dependency_last})
            list(GET _dependencies ${_dependency_index} _dependency)
            list(GET _dependency_scopes ${_dependency_index} _scope)
            ...
            if(NOT _guest_dependency STREQUAL "")
               ...
               if(_scope STREQUAL "PRIVATE")
                  list(APPEND _private_guest_dependencies "${_guest_dependency}")
               else()
                  list(APPEND _public_guest_dependencies "${_guest_dependency}")
               endif()
            else()
               get_target_property(_dependency_id "${_canonical}" FORGE_CONTRACT_LIBRARY_ID)
               list(APPEND _dependency_ids "${_dependency_id}")
               if(_scope STREQUAL "PRIVATE")
                  list(APPEND _private_dependency_ids "${_dependency_id}")
               else()
                  list(APPEND _public_dependency_ids "${_dependency_id}")
               endif()
            endif()
         endforeach()
      endif()
      list(REMOVE_DUPLICATES _dependency_ids)
      list(REMOVE_DUPLICATES _public_dependency_ids)
      list(REMOVE_DUPLICATES _private_dependency_ids)
      list(REMOVE_DUPLICATES _guest_dependencies)
      list(REMOVE_DUPLICATES _public_guest_dependencies)
      list(REMOVE_DUPLICATES _private_guest_dependencies)
      ...
      _forge_contract_append_set("${_path}" "${_prefix}_PUBLIC_DEPENDENCY_IDS" "${_public_dependency_ids}")
      _forge_contract_append_set("${_path}" "${_prefix}_PRIVATE_DEPENDENCY_IDS" "${_private_dependency_ids}")
      _forge_contract_append_set("${_path}" "${_prefix}_GUEST_DEPENDENCIES" "${_guest_dependencies}")
      _forge_contract_append_set(
         "${_path}" "${_prefix}_PUBLIC_GUEST_DEPENDENCIES" "${_public_guest_dependencies}"
      )
      _forge_contract_append_set(
         "${_path}" "${_prefix}_PRIVATE_GUEST_DEPENDENCIES" "${_private_guest_dependencies}"
      )
