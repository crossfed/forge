...
int main() {
   ...
   constexpr auto begin_revision_name =
      product::chain::begin_revision::get_name();
   host.invoke(
      code,
      account,
      account,
      begin_revision_name.value,
      forge::raw::pack(request)
   );

   const auto row = host.find_primary(
      account,
      request.workspace.value,
      protocol::make_name("revisions").value,
      0
   );
