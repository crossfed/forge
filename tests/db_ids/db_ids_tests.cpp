...
BOOST_AUTO_TEST_CASE(db_ids_typed_id_rejects_mismatched_space_or_type) {
   using account_id = forge::db::ids::typed_id<1, 2>;

   BOOST_CHECK_EXCEPTION(account_id(forge::db::ids::object_id{.space = 1, .type = 3, .instance = 99}),
   ...
   BOOST_CHECK(
       (!forge::db::ids::try_typed<1, 2>(forge::db::ids::object_id{.space = 9, .type = 2, .instance = 99}).has_value()));
}
