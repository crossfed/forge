...
class memory_session final : public forge::db::session {
 public:
   ...
   boost::asio::awaitable<std::optional<std::vector<std::byte>>> get(forge::db::family family,
                                                                     forge::db::record_key record) override {
   ...
   boost::asio::awaitable<void> put(forge::db::family family,
                                    forge::db::record_key record,
                                    std::vector<std::byte> value) override {
      working_[family.name][std::move(record)] = std::move(value);
      co_return;
   }

   boost::asio::awaitable<void> erase(forge::db::family family, forge::db::record_key record) override {
...
BOOST_AUTO_TEST_CASE(db_scan_pages_use_opaque_cursor_boundaries) {
   ...
   forge::asio::blocking::run(runtime, [&]() -> boost::asio::awaitable<void> {
      ...
      auto snapshot = co_await driver->begin_read();
      auto first = co_await snapshot.scan_page(meta, forge::db::record_range{.begin = key("a"), .end = key("z")}, {.limit = 2});
      ...
      auto second = co_await snapshot.scan_page(
         ...
         forge::db::page_request{.after = first.next, .limit = 2});
