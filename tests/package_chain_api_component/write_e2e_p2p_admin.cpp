...
module package.chain_api_component.write_e2e_p2p_admin;

import package.chain_api_component.p2p_runtime;
import package.chain_api_component.test_support;
import package.chain_api_component.write_e2e_p2p_admin_client;
import package.chain_api_component.write_e2e_p2p_admin_server;
...
write_p2p_admin_responses run_p2p_admin_e2e(std::shared_ptr<write_p2p_admin_fixture> state) {
   ...
   try {
      ...
      require(remote_apis.front().protocol == chain_api_protocol,
              "P2P resolver advertised admin on the wrong protocol");
      require(remote_apis.front().max_frame_size == chain_api_max_frame_size,
              "P2P resolver advertised the wrong admin frame limit");
      ...
      const auto responses = forge::asio::blocking::run(client.runtime(), run_p2p_admin_client(std::move(connection)));
