...
int main() {
   ...
   const auto p2p_response = run_p2p_e2e(p2p_services{
       ...
       .state_calls = [state = services.state] { return state->calls.load(std::memory_order_relaxed); },
       .transaction_await_started = [transactions = services.transactions] {
          return transactions->await_started.load(std::memory_order_acquire);
       },
       .transaction_await_deadlines = [transactions = services.transactions] {
          return transactions->await_deadlines.load(std::memory_order_acquire);
       },
       .transaction_await_cancellations = [transactions = services.transactions] {
          return transactions->await_cancellations.load(std::memory_order_acquire);
       },
   });
   ...
   require(p2p_response.information == forge::raw::pack(expected_info),
           "P2P info API changed chain audit DTO semantics");
   require(p2p_response.block == forge::raw::pack(expected_block), "P2P block API changed typed DTO semantics");
   require(p2p_response.state == forge::raw::pack(expected_state),
           "P2P state API changed chain audit DTO semantics");
   require(p2p_response.transaction == forge::raw::pack(expected_transaction),
           "P2P transaction API changed typed DTO semantics");
   require(p2p_response.administration == forge::raw::pack(expected_admin),
           "P2P admin API changed typed DTO semantics");
   require(p2p_response.oversized_request_rejected, "P2P Chain API request limit was not exercised");
   require(forge::raw::pack(http_response.information) == p2p_response.information,
           "HTTP and P2P info canonical bytes diverged");
   require(forge::raw::pack(http_response.block) == p2p_response.block,
           "HTTP and P2P block canonical bytes diverged");
   require(forge::raw::pack(http_response.state) == p2p_response.state,
           "HTTP and P2P state canonical bytes diverged");
   require(forge::raw::pack(http_response.transaction) == p2p_response.transaction,
           "HTTP and P2P transaction canonical bytes diverged");
   require(forge::raw::pack(http_response.administration) == p2p_response.administration,
