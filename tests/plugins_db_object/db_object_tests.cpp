...
import forge.plugins.db.object.api;
...
namespace object_plugin = forge::plugins::db::object;
...
struct byte_less {
   bool operator()(const forge::db::object::record_key& left, const forge::db::object::record_key& right) const {
      return left.bytes() < right.bytes();
   }
};

struct memory_state {
   std::map<forge::db::object::record_key, std::vector<std::byte>, byte_less> records;
...
class memory_session final : public forge::db::session {
 public:
   ...
   boost::asio::awaitable<std::optional<std::vector<std::byte>>> get(forge::db::family, forge::db::object::record_key key) override {
   ...
   boost::asio::awaitable<void> put(forge::db::family, forge::db::object::record_key key, std::vector<std::byte> value) override {
      if (!writes_) {
   ...
   boost::asio::awaitable<void> erase(forge::db::family, forge::db::object::record_key key) override {
      if (!writes_) {
         FORGE_THROW_EXCEPTION(forge::db::object::exceptions::unsupported_operation, "test snapshot is read-only");
      }
      working_.erase(key);
   ...
   boost::asio::awaitable<forge::db::object::record_page> scan_page(forge::db::family, forge::db::object::record_range range,
                                                                  forge::db::object::page_request request) override {
      forge::db::object::validate_page_request(request);

      auto result = forge::db::object::record_page{};
      ...
      auto last_returned = std::optional<forge::db::object::record_key>{};
      while (current != working_.end()) {
         if (range.has_end && !(current->first.bytes() < range.end.bytes())) {
            break;
         }
         result.entries.push_back(forge::db::object::record_entry{.key = current->first, .value = current->second});
      ...
      if (current != working_.end() && (!range.has_end || current->first.bytes() < range.end.bytes())) {
         result.next = forge::db::object::cursor{.boundary = std::move(*last_returned)};
      }
 ...
 private:
   ...
   bool writes_ = false;
   std::map<forge::db::object::record_key, std::vector<std::byte>, byte_less> working_;
};
...
class installer_plugin final : public forge::app::plugin {
 public:
   ...
   [[nodiscard]] forge::app::plugin_id id() const override {
      return forge::app::plugin_id{.value = "test.plugins.db.object.installer"};
   }
   ...
   boost::asio::awaitable<void> initialize(forge::app::plugin_context& context) override {
      auto api = context.apis().get<object_plugin::api>(object_plugin::api::ref());
...
[[nodiscard]] forge::app::plugin_descriptor installer_descriptor(std::shared_ptr<memory_driver> driver) {
   return forge::app::plugin_descriptor{
      .id = forge::app::plugin_id{.value = "test.plugins.db.object.installer"},
      .dependencies = {forge::app::plugin_id{.value = "forge.plugins.db.object"}},
...
make_app(forge::config::document document = {}, std::shared_ptr<memory_driver> driver = {}) {
   ...
   builder.name("db-object-plugin-test")
      .runtime(forge::asio::runtime_options{.worker_threads = 1, .thread_name = "db-object-plugin-test"})
      .plugin(object_plugin::descriptor());
   if (driver) {
...
[[nodiscard]] forge::config::document document_for_rocksdb(const std::filesystem::path& path) {
   ...
   document.set("plugins.db.object.stores", forge::config::value::array_type{configured_store("accounts", path)});
   return document;
}
...
BOOST_AUTO_TEST_SUITE(object_plugin_test_suite)

BOOST_AUTO_TEST_CASE(object_plugin_descriptor_api_and_config_are_nested) {
   auto plugin = object_plugin::plugin{};
   BOOST_TEST(plugin.id().value == "forge.plugins.db.object");
   BOOST_TEST(object_plugin::api::ref().id.value == "forge.plugins.db.object");

   const auto descriptor = plugin.describe_config();
   BOOST_REQUIRE(descriptor.has_value());
   BOOST_TEST(descriptor->section == "plugins.db.object");

   const auto& stores = require_field(*descriptor, "stores");
   BOOST_TEST(!stores.has_default);

   const auto api_descriptor = object_plugin::api::describe();
   BOOST_TEST(api_descriptor.id.value == "forge.plugins.db.object");
   BOOST_TEST(api_descriptor.methods.empty());
}

BOOST_AUTO_TEST_CASE(object_plugin_rejects_invalid_programmatic_setup) {
   ...
   auto document = forge::config::document{};
   forge::asio::blocking::run(runtime, plugin.configure(forge::config::component_view{document, "plugins.db.object"}));
   ...
   BOOST_CHECK_THROW(forge::asio::blocking::run(runtime, api->add_store("accounts", driver)),
                     object_plugin::exceptions::duplicate_store);
   ...
   BOOST_CHECK_THROW(forge::asio::blocking::run(runtime, api->add_store("late", driver)),
                     object_plugin::exceptions::stopped);

   forge::asio::blocking::run(runtime, plugin.shutdown());
}

BOOST_AUTO_TEST_CASE(object_plugin_rejects_duplicate_configured_store_names) {
   auto runtime = forge::asio::runtime{};
   auto plugin = object_plugin::plugin{};
   auto document = forge::config::document{};
   document.set(
      "plugins.db.object.stores",
      forge::config::value::array_type{
         configured_store("accounts", "/tmp/forge-db-object-plugin-duplicate-a"),
         configured_store("accounts", "/tmp/forge-db-object-plugin-duplicate-b"),
      });

   BOOST_CHECK_THROW(
      forge::asio::blocking::run(runtime, plugin.configure(forge::config::component_view{document, "plugins.db.object"})),
      object_plugin::exceptions::invalid_config);
}

BOOST_AUTO_TEST_CASE(object_plugin_rejects_configure_after_stop_or_shutdown) {
   auto runtime = forge::asio::runtime{};
   auto document = forge::config::document{};

   {
      ...
      BOOST_CHECK_THROW(forge::asio::blocking::run(runtime, plugin.startup()),
                        object_plugin::exceptions::startup_failed);
   }
...
BOOST_AUTO_TEST_CASE(object_plugin_custom_driver_store_handle_reads_writes_flushes_and_stops) {
   auto driver = std::make_shared<memory_driver>();
   auto app = make_app({}, driver);
   auto api = app->apis().get<object_plugin::api>(object_plugin::api::ref());
...
BOOST_AUTO_TEST_CASE(object_plugin_store_handle_remains_valid_during_dependent_shutdown) {
   auto runtime = forge::asio::runtime{};
   auto scheduler = forge::asio::task_scheduler{runtime};
   auto apis = forge::api::registry{};
   auto signals = forge::app::signal_bus{};
...
BOOST_AUTO_TEST_CASE(object_plugin_store_handle_concurrent_close_is_snapshot_safe) {
   auto runtime = forge::asio::runtime{};
   auto scheduler = forge::asio::task_scheduler{runtime};
   auto apis = forge::api::registry{};
   auto signals = forge::app::signal_bus{};
   ...
   do {
      try {
         ...
         ++successes;
      } catch (const object_plugin::exceptions::stopped&) {
         ++stopped;
      }
...
BOOST_AUTO_TEST_CASE(object_plugin_unknown_store_fails_typed) {
   auto app = make_app();
   auto api = app->apis().get<object_plugin::api>(object_plugin::api::ref());

   BOOST_CHECK_THROW(forge::asio::blocking::run(app->runtime(), api->store("missing")),
                     object_plugin::exceptions::unknown_store);
...
#if FORGE_HAS_ROCKSDB
BOOST_AUTO_TEST_CASE(object_plugin_configured_rocksdb_store_persists_across_reopen) {
