...
#define FORGE_VM_WASM_INTERPRET_TEST_FILE vector_tests
...
TEST_CASE( "unmanaged_vector tests", "[unmanaged_vector_tests]") {
   {
      ...
      for (int i = 0; i < uv0.size(); i++)
         uv0[i] = 'a';
      BOOST_CHECK_THROW([&](){ uv0.at(10) = 'a'; }(), std::exception);
      uv0.resize(11);
      uv0[10] = 'a';
      BOOST_CHECK_THROW([&](){ uv0.at(11) = 'a'; }(), std::exception);
      uv0.resize(8);
      BOOST_CHECK_THROW([&](){ uv0.at(8) = 'a'; }(), std::exception);
      size_t uv0_size = uv0.size();
      unmanaged_vector<char> uv1(std::move(uv0));
      BOOST_TEST(static_cast<bool>( uv0_size == uv1.size() ));
...
TEST_CASE( "managed_vector tests", "[managed_vector_tests]") {
   ...
   for (int i = 0; i < v0.size(); i++)
      v0[i] = 'a';
   BOOST_CHECK_THROW([&](){ v0[10] = 'a'; }(), std::exception);
   v0.resize(10);
   v0[9] = 'a';
   BOOST_CHECK_THROW([&](){ v0[10] = 'a'; }(), std::exception);
   v0.resize(8);
   BOOST_CHECK_THROW([&](){ v0[8] = 'a'; }(), std::exception);
   size_t v0_size = v0.size();
   managed_vector<char, bounded_allocator> v1(std::move(v0));
   BOOST_TEST(static_cast<bool>( v0_size == v1.size() ));
   for (int i=0; i < v1.size(); i++)
      v1[i] = 'b';
   v1.emplace_back('b');
   for (int i=0; i < 7; i++)
      v1.emplace_back('c');
   BOOST_CHECK_THROW([&]() {v1.emplace_back('c');}(), std::exception);
}
