...
void check_put(B& bkend, const std::string& name) {
   for(auto fun : fun_prefixes) {
      ...
      for(const T value : test_values<T>) {
         ...
         BOOST_TEST(static_cast<bool>(global_test_value<T> == value));
      }
   }
}
...
void check_put_ref(B& bkend, const std::string& name) {
   for(auto fun : fun_prefixes) {
      ...
      bkend.call("env", fun, value);
      BOOST_TEST(static_cast<bool>(global_test_value<T>.val == value));
   }
}
...
void check_get(B& bkend, const std::string& name, F getter) {
   for(auto fun : fun_prefixes) {
      ...
      for(const T value : test_values<T>) {
         ...
         BOOST_TEST(static_cast<bool>(getter(bkend.call_with_return("env", fun)) == value));
      }
   }
}
...
void check_get_ptr(B& bkend, const std::string& name) {
   for(auto fun : fun_prefixes) {
      ...
      BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", fun)->to_ui32() == offset));
   }
}
...
void test_parameters(Backend&& bkend) {
   for(auto fun : fun_prefixes) {
      ...
      BOOST_TEST(static_cast<bool>(global_test_value<bool> == true));
      bkend.call("env", fun, false);
      BOOST_TEST(static_cast<bool>(global_test_value<bool> == false));
...
void test_results(Backend&& bkend) {
   for(auto fun : {/*"get_b",*/ "call.get_b", "call_indirect.get_b"}) {
      ...
      BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", fun)->to_ui32() == 0u));
      global_test_value<bool> = true;
      BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", fun)->to_ui32() == 1u));
   }
...
BACKEND_TEST_CASE( "Test C-style host function system", "[C-style_host_functions_tests]") {
   ...
   BOOST_TEST(static_cast<bool>(c_style_host_function_state == 1));

   bkend.call("env", "apply", (uint64_t)1, (uint64_t)2, (uint64_t)0);
   BOOST_TEST(static_cast<bool>(c_style_host_function_state == 2));

   bkend.call("env", "apply", (uint64_t)2, (uint64_t)1, (uint64_t)2);
   BOOST_TEST(static_cast<bool>(c_style_host_function_state == 3));

   float f = 2.4f;
   bkend.call("env", "apply", (uint64_t)3, (uint64_t)2, (uint64_t)bit_cast<uint32_t>(f));
   BOOST_TEST(static_cast<bool>(c_style_host_function_state == 0x40199980));

   bkend.call("env", "apply", (uint64_t)4, (uint64_t)5, (uint64_t)bit_cast<uint32_t>(f));
   BOOST_TEST(static_cast<bool>(c_style_host_function_state == 5));
}
...
BACKEND_TEST_CASE( "Testing host functions", "[host_functions_test]" ) {
   ...
   BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", "test", UINT32_C(5))->to_i32() == 49));
   BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", "test.indirect", UINT32_C(5), UINT32_C(0))->to_i32() == 47));
   BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", "test.indirect", UINT32_C(5), UINT32_C(1))->to_i32() == 210));
   BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", "test.indirect", UINT32_C(5), UINT32_C(2))->to_i32() == 49));
   BOOST_CHECK_THROW(bkend.call("env", "test.indirect", UINT32_C(5), UINT32_C(3)), std::exception);
   BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", "test.local-call", UINT32_C(5))->to_i32() == 147));
}
...
BACKEND_TEST_CASE( "Testing multi ", "[host_functions_multi_converter]") {
   ...
   */
   ...
   BOOST_TEST(static_cast<bool>(bkend.call_with_return("env", "test", UINT32_C(1), UINT32_C(2), UINT32_C(3), UINT32_C(4))->to_i32() == 4321));
   BOOST_TEST(static_cast<bool>(multi_converter_destructor_order == std::vector{1, 3}));
}
...
BACKEND_TEST_CASE( "Testing throwing host functions", "[host_functions_throw_test]" ) {
   ...
   BOOST_CHECK_THROW(bkend.call("env", "test", UINT32_C(2)), test_exception);
}
...
BACKEND_TEST_CASE( "Testing exiting host functions", "[host_functions_exit_test]" ) {
   ...
   BOOST_TEST(static_cast<bool>(!bkend.call_with_return(host, "env", "test", UINT32_C(2))));
}
