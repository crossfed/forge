...
import forge.vm.wasm.interpret.variant;
#include "test_support.hpp"

#define FORGE_VM_WASM_INTERPRET_TEST_FILE variant_tests
...
TEST_CASE("Testing variant with stateless class", "[variant_stateless_tests]") {
...

   {
      ...
      BOOST_TEST(static_cast<bool>(visit(vis{}, v) == 43));
   }
   {
      variant_type v((short)142);
      BOOST_TEST(static_cast<bool>(visit(vis{}, v) == 144));
   }
   {
      variant_type v((int)242);
      BOOST_TEST(static_cast<bool>(visit(vis{}, v) == 245));
   }
   {
      variant_type v((long)342);
      BOOST_TEST(static_cast<bool>(visit(vis{}, v) == 346));
   }
   {
      variant_type v((float)442);
      BOOST_TEST(static_cast<bool>(visit(vis{}, v) == 447));
   }
   {
      variant_type v((double)542);
      BOOST_TEST(static_cast<bool>(visit(vis{}, v) == 548));
   }
}
...
TEST_CASE("Testing argument forwarding for visit", "[variant_forward_tests]") {
   ...
   // lvalue
   {
      ...
      vis f;
      visit(f, v);
      BOOST_TEST(static_cast<bool>(f.r == 43));
   }
   // rvalue
   {
      struct vis {
   ...
   // lvalue
   {
      ...
      variant_type v((double)42);
      visit(vis{}, v);
      BOOST_TEST(static_cast<bool>(v.get<double>() == 43));
   }
...
TEST_CASE("Testing constexpr variant", "[variant_constexpr_tests]") {
...

   {
      ...
      BOOST_TEST(static_cast<bool>(result_lv == 2));
      constexpr signed char result_rv{variant_type{(signed char)4}.get<signed char>()};
      BOOST_TEST(static_cast<bool>(result_rv == 4));
   }
   {
...
#ifdef CONSTEXPR_STD_INVOKE
   {
...

      {
         ...
         BOOST_TEST(static_cast<bool>(test == 3));
      }
   }
#endif
}
