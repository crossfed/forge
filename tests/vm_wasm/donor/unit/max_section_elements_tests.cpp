...
#define FORGE_VM_WASM_INTERPRET_TEST_FILE max_section_elements_tests
...
BACKEND_TEST_CASE("Test max_" #name "_section_elements", "[max_section_elements_test]") {                                    \
   ...
   BOOST_CHECK_THROW((backend<std::nullptr_t, TestType, static_options_1>(wasm, &wa)), exceptions::parse);                  \
   backend<std::nullptr_t, TestType, static_options_2> backend_static_fallback(wasm, &wa);                                   \
   BOOST_CHECK_THROW((backend<std::nullptr_t, TestType, dynamic_options>(wasm, nullptr, {1})), exceptions::parse);          \
   backend<std::nullptr_t, TestType, dynamic_options> backend_dynamic_fallback(wasm, nullptr, {2});                          \
                                                                                                                             \
   BOOST_CHECK_THROW((backend<std::nullptr_t, TestType, name ## _static_options_1>(wasm, &wa)), exceptions::parse);         \
   backend<std::nullptr_t, TestType, name ## _static_options_2> backend_static(wasm, &wa);                                   \
   BOOST_CHECK_THROW((backend<std::nullptr_t, TestType, name ## _dynamic_options>(wasm, nullptr, {1})), exceptions::parse); \
