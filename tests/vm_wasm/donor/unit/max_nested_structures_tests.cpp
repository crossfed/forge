...
#define FORGE_VM_WASM_INTERPRET_TEST_FILE max_nested_structures_tests
...
BACKEND_TEST_CASE("Test max_nested_structures static fail", "[max_nested_structures_test]") {
   ...
   BOOST_CHECK_THROW(backend_t(depth_2_wasm, &wa), exceptions::parse);
}
...
BACKEND_TEST_CASE("Test max_nested_structures dynamic fail", "[max_nested_structures_test]") {
   ...
   BOOST_CHECK_THROW(backend_t(depth_2_wasm, nullptr, dynamic_options{1}), exceptions::parse);
}
