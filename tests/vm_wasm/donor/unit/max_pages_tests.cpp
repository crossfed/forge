...
#define FORGE_VM_WASM_INTERPRET_TEST_FILE max_pages_tests
...
BACKEND_TEST_CASE("Test max_pages static fail", "[max_pages_test]") {
   ...
   BOOST_CHECK_THROW(backend_t(mem_wasm, &wa), exceptions::parse);
}
...
BACKEND_TEST_CASE("Test max_pages static pass", "[max_pages_test]") {
   ...
   backend_t backend(mem_wasm, &wa);
   BOOST_TEST(static_cast<bool>(backend.call_with_return("env", "f")->to_i32() == -1));
}
...
BACKEND_TEST_CASE("Test max_pages dynamic fail", "[max_pages_test]") {
   ...
   BOOST_CHECK_THROW(backend_t(mem_wasm, nullptr, dynamic_options{2}), exceptions::parse);
}
...
BACKEND_TEST_CASE("Test max_pages dynamic pass", "[max_pages_test]") {
   ...
   BOOST_TEST(static_cast<bool>(backend.call_with_return("env", "f")->to_i32() == 3));
   backend.initialize(nullptr, dynamic_options{3});
   BOOST_CHECK_THROW(backend.initialize(nullptr, dynamic_options{2}), std::exception);
}
