...
#define FORGE_VM_WASM_INTERPRET_TEST_FILE max_local_sets_tests
...
BACKEND_TEST_CASE("Test max_local_sets static fail", "[max_local_sets_test]") {
   ...
   BOOST_CHECK_THROW(backend_t(two_sets_wasm, &wa), exceptions::parse);
}
...
BACKEND_TEST_CASE("Test max_local_sets dynamic fail", "[max_local_sets_test]") {
   ...
   BOOST_CHECK_THROW(backend_t(two_sets_wasm, nullptr, dynamic_options{1}), exceptions::parse);
}
